...
double IQPTree::doVNS()
{
	curScore = optimizeNNI(false);
	return curScore;
}

bool IQPTree::containPosNNI(vector<NNIMove> posNNIs)
{
...
void IQPTree::applyNNIs(int nni2apply, bool neg_nni) {
...
bool IQPTree::checkBootstrapStopping() {
	if (boot_splits.size() < 2) return false;
...
void IQPTree::printResultTree(ostream &out) {
    setRootNode(params->root);
    printTree(out, WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH | WT_SORT_TAXA);
}

void IQPTree::printIntermediateTree(int brtype) {
	setRootNode(params->root);
