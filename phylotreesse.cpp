...
void PhyloTree::computePartialLikelihoodEigen(PhyloNeighbor *dad_branch, PhyloNode *dad) {
    ...
    FOR_NEIGHBOR_IT(node, dad, it) {
        ...
        for (c = 0; c < ncat; c++) {
            ...
            eval_ptr += (model_factory->fused_mix_rate * nstates);
            evec_ptr += (model_factory->fused_mix_rate * nstatesqr);
        }
        ...
        if (child->node->isLeaf()) {
            ...
            for (it = aln->seq_states[child->node->id].begin(); it != aln->seq_states[child->node->id].end(); it++) {
                ...
                for (c = 0; c < ncat; c++) {
                    ...
                    this_tip_partial_lh += model_factory->fused_mix_rate*nstates;
                }
            }
...
//                            double *echild_ptr = echild + (c*nstatesqr+x*nstates);
            ...
            for (c = 0; c < ncat; c++) {
                ...
                inv_evec_ptr -= (!model_factory->fused_mix_rate)*nstatesqr;
            }
...
#pragma omp parallel for private(ptn, i, c) schedule(static)
	    	...
	    	for (ptn = 0; ptn < nptn; ptn++) {
				...
				double *lh_tip = tip_partial_lh + ((int)((ptn < orig_nptn) ? (aln->at(ptn))[dad->id] :  model_factory->unobserved_ptns[ptn-orig_nptn]))*tip_block;
                for (c = 0; c < ncat; c++) {
                    for (i = 0; i < nstates; i++) {
                    ...
                    theta += nstates;
                    lh_tip += model_factory->fused_mix_rate*nstates;
                }
...
#pragma omp parallel for private(ptn, i) schedule(static)
	...
	for (c = 0; c < ncat; c++) {
		...
		for (i = 0; i < nstates; i++) {
			double cof = eval_ptr[i]*site_rate->getRate(c);
...
//        computePartialLikelihoodEigen(node_branch, node);
	...
	for (c = 0; c < ncat; c++) {
		...
		double prop = site_rate->getProp(c);
		for (i = 0; i < nstates; i++)
			val[c*nstates+i] = exp(eval_ptr[i]*len) * prop;
        eval_ptr += model_factory->fused_mix_rate*nstates;
	}
