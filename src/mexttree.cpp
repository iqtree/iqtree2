...
void MExtTree::calcDist(matrix(double) &dist, Node *node, Node *dad) {
	if (!node) node = root;
	if (node->isLeaf()) {
		calcDist(node, 0, dist, node, NULL);
	}
	//for (NeighborVec::iterator it = node->neighbors.begin(); it != node->neighbors.end(); it++) 
		//if ((*it)->node != dad)	{
...
void MExtTree::calcDist(Node *aroot, double cur_len, matrix(double) &dist, Node *node, Node *dad) {
	if (!node) node = root;
	if (node->isLeaf()) {
		dist[aroot->id][node->id] = cur_len;
		dist[node->id][aroot->id] = cur_len;
	}
	//for (NeighborVec::iterator it = node->neighbors.begin(); it != node->neighbors.end(); it++) 
		//if ((*it)->node != dad)	{
	FOR_NEIGHBOR_IT(node, dad, it) {
