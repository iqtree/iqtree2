    ...
    for (cur_iteration = 2; !stop_rule.meetStopCondition(cur_iteration); cur_iteration++) {
        ...
        if (enableHeuris) {
            if (cur_iteration > SPEED_UP_AFTER_ITER_NUM) {
                ...
                double predictScore = iqp_score + nni_count_est * nni_delta_est;
                if (predictScore >= bestScore + TOL_LIKELIHOOD)
                    optimizeNNI();
                else {
                    /* take back the current best tree */
                    best_tree_string.seekg(0);
...
double IQPTree::optimizeNNI() {
    ...
    do {
        ...
        nbNNIToApply = (int) nniTotal * curLambda;
        if (nbNNIToApply < 1)
            nbNNIToApply = 1;
        ...
        if (newScore > curScore + TOL_LIKELIHOOD) {
            ...
            if (enableHeuris) {
                vecImpProNNI.push_back((newScore - curScore) / nbNNIToApply);
...
//            }
            ...
            cout << "The tree didn't improve at NNI step " << nniIteration
                    ...
                    << ") -> Trying new lamda = " << curLambda << endl;
            //Tree cannot be worse if only 1 NNI is applied
            assert((nbNNIToApply - 1) != 0); 
...
double IQPTree::applyBranchLengthChange(PhyloNode *node1, PhyloNode *node2,
    ...
    if (nonNNIBranch) {
        new_len = current_len + startLambda * (optLen - current_len);
    } else {
        new_len = optLen;
    }
...
//        node21_it->partial_lh = newPartialLh();
    ...
    FOR_NEIGHBOR_IT(node2, node1, node2_it) {
        ...
        // This is not efficient ! We should recover the optimized branch length 
