...
void reportReferences(Params &params, ofstream &out) {
    ModelInfoFromName info(params.model_name);
...
void reportModel(ofstream &out, Alignment* aln, ModelSubst* m) {
    ...
    if (m->num_states <= 4) {
        ...
        if (m->isReversible()) {
            for (int i = 0, k = 0; i < m->num_states - 1; ++i) {
                for (int j = i + 1; j < m->num_states; ++j, ++k) {
                    out << "  " << aln->convertStateBackStr(i)
                        << "-"  << aln->convertStateBackStr(j)
                        << ": " << rate_mat[k];
                    if (m->num_states <= 4) {
                        out << endl;
                    }
            ...
            for (int i = 0, k = 0; i < m->num_states; ++i) {
        ...
        out << "WARNING: This model has " << m->getNDim() + m->getNDimFreq()
            ...
            << " Please use with caution!" << endl << endl;
        double full_mat[400];
        ...
        if (m->isReversible()) {
            ...
            out << "Substitution parameters (lower-diagonal)"
                << " and state frequencies in PAML format"
                << " (can be used as input for IQ-TREE): " << endl << endl;
            for (int i = 1; i < m->num_states; ++i) {
                for (int j = 0; j < i; ++j) {
            ...
            out << "Full Q matrix and state frequencies"
                << " (can be used as input for IQ-TREE): " << endl << endl;
...
//        else
            ...
            case StateFreqType::FREQ_USER_DEFINED:
                out << ((aln->seq_type == SeqType::SEQ_PROTEIN) 
                       ? "(model)" : "(user-defined)") << endl;
                break;
        ...
        auto freq = m->getFreqType();

        if ((freq != StateFreqType::FREQ_USER_DEFINED || 
             aln->seq_type == SeqType::SEQ_DNA) &&
        ...
        if (m->num_states <= 4 || 
            ...
            for (int i = 0, k = 0; i < m->num_states; ++i) {
                out << "  " << aln->convertStateBackStr(i);
                for (int j = 0; j < m->num_states; ++j, ++k) {
                    out << "  ";
                    out.width(8);
                    out << q_mat[k];
                }
...
//        out << "Full name: " << tree.getModelName() << endl;
        ...
        for (int i = 0; i < nmix; i++) {
            ...
            out.width(7);
            out << mmodel->getMixtureWeight(i) 
                << "  " << (m)->getNameParams() << endl;
...
void reportRate(ostream &out, PhyloTree &tree) {
    ...
    if (rate_model->getNDiscreteRate() > 1 ||
        rate_model->getPInvar() > 0.0) {
        out << endl << " Category  Relative_rate  Proportion" << endl;
        if (rate_model->getPInvar() > 0.0) {
            out << "  0         0              " 
                << rate_model->getPInvar() << endl;
        }
        int cats = rate_model->getNDiscreteRate();
        DoubleVector prop;
        if (rate_model->getGammaShape() > 0 || 
            rate_model->getPtnCat(0) < 0) {
            // prop.resize(cats, (1.0 - rate_model->getPInvar())
            //                   / rate_model->getNRate());
            prop.resize(cats);
            ...
            if (frequencies!=nullptr) {
                for (size_t i = 0; i < num_patterns; i++) {
                    ...
                    auto bump = static_cast<double>(frequencies[i]);
                    prop[rate_model->getPtnCat(static_cast<int>(i))] += bump;
                }
            } else {
                for (size_t i = 0; i < num_patterns; i++) {
                    //This won't work if >2 billion patterns
                    auto bump = static_cast<double>(tree.aln->at(i).frequency);
                    prop[rate_model->getPtnCat(static_cast<int>(i))] += bump;
                }
            }
            for (int i = 0; i < cats; i++) {
        ...
        if (rate_model->isGammaRate()) {
            bool want_median = (rate_model->isGammaRate() == GAMMA_CUT_MEDIAN);
            out << "Relative rates are computed as "
                << ( want_median ? "MEDIAN" : "MEAN")
                << " of the portion of the Gamma distribution"
                << " falling in the category." << endl;
        }
    }
    /*
     if (rate_model->getNDiscreteRate() > 1 || rate_model->isSiteSpecificRate())
     out << endl << "See file " << rate_file
         << " for site-specific rates and categories" << endl;
     */
    out << endl;
}
...
void reportTree(ofstream &out, Params &params, PhyloTree &tree,
    ...
    auto   factory  = tree.getModelFactory();
    int    df       = factory->getNParameters(BRLEN_OPTIMIZE);
    ...
    out << endl;
    out << "Unconstrained log-likelihood (without tree): "
        << tree.aln->computeUnconstrainedLogL() << endl;

    out << "Number of free parameters" 
        << " (#branches + #model parameters): " << df << endl;
//    if (ssize > df) {
//        if (ssize > 40*df)
//            out << "Akaike information criterion" 
//                << " (AIC) score: " << AIC_score << endl;
//        else
//            out << "Corrected Akaike information criterion"
//                << " (AICc) score: " << AICc_score << endl;
//
//        out << "Bayesian information criterion" 
//            << " (BIC) score: " << BIC_score << endl;
//    } else
    out << "Akaike information criterion (AIC) score: " 
        << AIC_score << endl;
    out << "Corrected Akaike information criterion (AICc) score: " 
        << AICc_score << endl;
    out << "Bayesian information criterion (BIC) score: " 
        << BIC_score << endl;
    ...
    if (tree.aln->seq_type == SeqType::SEQ_POMO) {
      ...
      double totLenIntSub = totalLenInternal/(N * N);
        ...
        out << "- measured in mutations and frequency shifts" 
            << " per site: " << totalLenInternal
            << " (" << totalLenInternalP << "% of tree length)" << endl;
        out << "- measured in substitutions per site: " << totLenIntSub
            << " (" << totalLenInternalP << "% of tree length)" << endl;
        out << endl;
    }
    else {
        out << "Sum of internal branch lengths: "
            << totalLenInternal << " (" << totalLenInternalP
            << "% of tree length)" << endl;
    ...
    if (params.partition_type == TOPO_UNLINKED) {
        out << "Tree topologies are unlinked across partitions,"
            << " thus no drawing will be displayed here" << endl;
        out << endl;
        return;
    }
    ...
    if (zero_internal_branches > 0) {
        //int zero_internal_branches = 
        //    ree.countZeroInternalBranches(NULL, NULL, epsilon);
        /*
        out << "WARNING: " << zero_branches
                << " branches of near-zero lengths (<" << epsilon << ")" 
                << " and should be treated with caution!"
                << endl;
        */
        out << "WARNING: " << zero_internal_branches
            << " near-zero internal branches (<" << epsilon << ")"
        ...
        cout << endl << "WARNING: " << zero_branches
                ...
                << endl;
        */
        out << "         Such branches are denoted by '**'"
            << " in the figure below"
            << endl << endl;
    }
    ...
    if (long_branches > 0) {
        ...
        out << "WARNING: " << long_branches
            << " too long branches (>" << params.max_branch_length-0.2 << ")"
    ...
    tree.sortTaxa();
    if (tree.rooted) {
        out << "NOTE: Tree is ROOTED at virtual root" 
            << " '" << tree.root->name << "'" << endl;
    }
    else {
        out << "NOTE: Tree is UNROOTED although outgroup taxon" 
            << " '" << tree.root->name << "'"
            << " is drawn at root" << endl;
    }
    if (tree.isSuperTree() && params.partition_type == BRLEN_OPTIMIZE) {
        out << "NOTE: Branch lengths are weighted average" 
            << " over all partitions" << endl
            << "      (weighted by the number of sites" 
            << " in the partitions)" << endl;
    }
    if (tree.isMixlen()) {
        out << "NOTE: Branch lengths are weighted average" 
            << " over heterotachy classes" << endl;
    }
    ...
    if (main_tree) {
        if (params.aLRT_replicates > 0 || params.gbo_replicates ||
            ...
            if (params.num_bootstrap_samples && params.compute_ml_tree) {
                if (params.aLRT_replicates > 0 || params.aLRT_test || 
                    params.aBayes_test) {
                    out << " /";
                }
    ...
    if (tree.isMixlen()) {
        out << " (class branch lengths are given" 
            << " in [...] and separated by '/' )";
    }
    if (tree.aln->seq_type == SeqType::SEQ_POMO) {
        out << " (measured in mutations and frequency shifts)";
  ...
  if (tree.aln->seq_type == SeqType::SEQ_POMO) {
    out << "Tree in newick format" 
        << " (measured in substitutions, see above):" << endl
        << "WARNING: Only for comparison with substitution models." << endl
        << "         These are NOT the branch" 
        << " lengths inferred by PoMo." << endl << endl;
    double len_scale_old = tree.len_scale;
    int N = tree.aln->virtual_pop_size;
    tree.len_scale = 1.0/(N*N);
    tree.printTree(out, WT_BR_SCALE | WT_BR_LEN |
                   WT_BR_LEN_FIXED_WIDTH | WT_SORT_TAXA);
...
void reportCredits(ofstream &out) {
    out << "CREDITS" << endl << "-------" << endl << endl
            << "Some parts of the code were taken" 
            << " from the following packages/libraries:"
            << endl << endl
            ...
            << " SPRNG: A Scalable Library" << endl
            << "for Pseudorandom Number Generation."
            << " ACM Transactions on Mathematical Software," << endl
            << "26: 436-461." << endl << endl

            << "Guennebaud G, Jacob B, et al. (2010) Eigen v3." 
            << " http://eigen.tuxfamily.org" << endl << endl;
            /*
...
void printOutfilesInfo(Params &params, IQTree &tree) {
    ...
    if (!(params.suppress_output_flags & OUT_IQTREE)) {
        cout << "  IQ-TREE report:                " 
             << params.out_prefix << ".iqtree"
             << endl;
    }
    if (params.compute_ml_tree) {
        if (!(params.suppress_output_flags & OUT_TREEFILE)) {
            if (model_info.isModelFinderOnly())
                cout << "  Tree used for ModelFinder:     "
                     << params.out_prefix << ".treefile" << endl;
            else {
                cout << "  Maximum-likelihood tree:       "
                     << params.out_prefix << ".treefile" << endl;
                if (params.partition_type == BRLEN_OPTIMIZE && 
                    tree.isSuperTree()) {
                    cout << "  Partition trees:               "
                         << params.out_prefix << ".parttrees" << endl;
                }
            }
        }
        //if (params.snni && params.write_local_optimal_trees) {
        //    cout << "  Locally optimal trees ("
        //         << tree.candidateTrees.getNumLocalOptTrees()
    ...
    if (!params.dist_file) {
        //cout << "  Jukes-Cantor distances:    "
        //     << params.out_prefix << ".jcdist" << endl;
        if (params.compute_ml_dist) {
            cout << "  Likelihood distances:          "
                 << params.out_prefix << ".mldist" << endl;
        }
        if (params.print_conaln) {
            cout << "  Concatenated alignment:        "
    ...
    if (params.compute_likelihood && params.print_site_rate) {
        if (tree.getRate()->getGammaShape() > 0 ||
            params.partition_file) {
            cout << "  Site-specific rates:           "
                 << params.out_prefix << ".rate" << endl;
        }
        if (tree.getRate()->isSiteSpecificRate() ||
            tree.getRate()->getPtnCat(0) >= 0) {
            cout << "  Site-rates by MH model:        "
                 << params.out_prefix << ".rate" << endl;
        }
    }
    if (params.print_site_lh) {
        cout << "  Site log-likelihoods:          "
             << params.out_prefix << ".sitelh" << endl;
    }
    if (params.print_partition_lh) {
        cout << "  Partition log-likelihoods:     "
             << params.out_prefix << ".partlh" << endl;
    }
    if (params.print_site_prob) {
        cout << "  Site probability per rate/mix: "
             << params.out_prefix << ".siteprob" << endl;
    }
    if (params.print_ancestral_sequence) {
        cout << "  Ancestral state:               "
             << params.out_prefix << ".state" << endl;
        //cout << "  Ancestral sequences:           "
        //     << params.out_prefix << ".aseq" << endl;
    }
    if (params.write_intermediate_trees) {
        cout << "  All intermediate trees:        "
             << params.out_prefix << ".treels" << endl;
    }
    if (params.writeDistImdTrees) {
        tree.intermediateTrees.printTrees(string("ditrees"));
        cout << "  Distinct intermediate trees:   "
             << params.out_prefix <<  ".ditrees" << endl;
        cout << "  Logl of intermediate trees:    "
             << params.out_prefix <<  ".ditrees_lh" << endl;
    }
    if (params.gbo_replicates) {
        cout << endl << "Ultrafast " << RESAMPLE_NAME
             << " approximation results written to:" << endl;
        if (!tree.isSuperTreeUnlinked()) {
            cout << "  Split support values:          "
                 << params.out_prefix << ".splits.nex" << endl
                 << "  Consensus tree:                "
                 << params.out_prefix << ".contree" << endl;
        }
        if (params.print_ufboot_trees) {
            cout << "  UFBoot trees:                  "
                 << params.out_prefix << ".ufboot" << endl;
        }
    }

    if (!params.treeset_file.empty()) {
        cout << "  Evaluated user trees:          "
             << params.out_prefix << ".trees" << endl;
        if (params.print_tree_lh) {
            cout << "  Tree log-likelihoods:          "
                 << params.out_prefix << ".treelh" << endl;
        }
    }
    if (params.lmap_num_quartets >= 0) {
        cout << "  Likelihood mapping plot (SVG): "
             << params.out_prefix << ".lmap.svg" << endl;
        cout << "  Likelihood mapping plot (EPS): "
             << params.out_prefix << ".lmap.eps" << endl;
    }
    if (!(params.suppress_output_flags & OUT_LOG)) {
        cout << "  Screen log file:               "
             << params.out_prefix << ".log" << endl;
    }
    cout << endl;
}

void reportPhyloAnalysis(Params &params, IQTree &tree,
    ...
    if (params.count_trees) {
        ...
        cout << endl << "NOTE: " << pllTreeCounter.size()
             << " distinct trees evaluated during whole tree search" << endl;

        IntVector counts;
        for (StringIntMap::iterator i = pllTreeCounter.begin();
             i != pllTreeCounter.end(); ++i) {
            if (i->second > counts.size())
                counts.resize(i->second+1, 0);
            counts[i->second]++;
        }
        for (auto i2 = counts.begin(); i2 != counts.end(); ++i2) {
            if (*i2 != 0) {
                cout << "#Trees occurring " << (i2-counts.begin())
                     << " times: " << *i2 << endl;
            }
        }
    }
    ...
    try {
        ...
        out << "IQ-TREE " << iqtree_VERSION_MAJOR 
            << "." << iqtree_VERSION_MINOR
            << iqtree_VERSION_PATCH << " COVID-edition built " 
            << __DATE__ << endl << endl;
        if (params.partition_file) {
            out << "Partition file name: " 
                << params.partition_file << endl;
        }
        if (params.aln_file) {
            out << "Input file name: " 
                << params.aln_file << endl;
        }
        if (!params.user_file.empty()) {
            out << "User tree file name: " 
                << params.user_file << endl;
        }
        out << "Type of analysis: ";
        ModelInfoFromName model_info(params.model_name);
        bool modelfinder = model_info.isModelFinder();
        if (modelfinder)
            out << "ModelFinder";
        if (params.compute_ml_tree) {
            if (modelfinder) {
        ...
        if (params.gbo_replicates > 0) {
            out << " + ultrafast " << RESAMPLE_NAME
                << " (" << params.gbo_replicates << " replicates)";
        }
        out << endl;
      ...
      // TODO DS: Changes may be needed here for PoMo.
            ...
            for (PhyloTree* part_tree : *stree)  {
                auto part_aln    = part_tree->aln; //alignment for partition [part]
                auto part_nsite  = part_tree->getAlnNSite();
                auto const_nsite = int(floor(part_aln->frac_const_sites*part_nsite+.5));
                ...
                out << left << part_aln->name << "  ";
                out << getSeqTypeShortName(part_aln->seq_type, true);
                out << "\t" << part_aln->getNSeq() 
                    << "\t" << part_aln->getNSite()
                    << "\t" << part_aln->getNPattern() 
                    << "\t" << part_aln->num_informative_sites
                    << "\t" << part_nsite - part_aln->num_variant_sites
                    << "\t" << const_nsite 
                    << endl;
                ++part;
            }
            out << endl << "Column meanings:" << endl
                ...
                << "  Const:  Number of constant sites" 
                << " (can be subset of invariant sites)" << endl << endl;

        }
        else {
            int removed_count = static_cast<int>(tree.removed_seqs.size());
            reportAlignment(out, *(tree.aln), removed_count);
        }
...
//            }
        ...
        if (tree.isSuperTree()) {
            ...
            for (PhyloTree* part_tree : *stree) {
                ...
                if (params.partition_type == BRLEN_OPTIMIZE ||
                    params.partition_type == TOPO_UNLINKED) {
                    out << left << part_tree->getModelName()
                        << " "  << part_tree->treeLength()
                        << "  " << part_tree->getModelNameParams() << endl;
                }
                else {
                    out << left << part_tree->getModelName()
            ...
            for (auto itm = part_model->linked_models.begin();
                ...
                for (PhyloTree* part_tree : *stree) {
                    if (part_tree->getModel() == itm->second) {
                        out << "Linked model of substitution: "
                            << itm->second->getName() << endl << endl;
                        bool fixed = part_tree->getModel()->fixParameters(false);
                        reportModel(out, part_tree->aln, part_tree->getModel());
                        part_tree->getModel()->fixParameters(fixed);
                        break;
                    }
                }
            }
        ...
        */
        ...
        if (model_info.isWeissAndVonHaeselerTest()) {
            ...
            if (params.whtest_p_value < 0.05) {
                out << "RESULT: Homogeneity assumption"
                    << " is rejected (p-value cutoff 0.05)"
                    << endl;
            } else {
                out << "RESULT: Homogeneity assumption"
                    << " is NOT rejected (p-value cutoff 0.05)"
                    << endl;
            }
            out << endl << "*** For this result please cite:" << endl << endl;
            out
                    << "G. Weiss and A. von Haeseler (2003)" 
                    << " Testing substitution models"
                    << endl
                    << "within a phylogenetic tree."
                    << " Mol. Biol. Evol, 20(4):572-578"
                    << endl << endl;
        }
        ...
        if (params.compute_ml_tree) {
            ...
            if (params.gbo_replicates) {
                if (tree.boot_consense_logl > tree.getBestScore() + 0.1 &&
                    !tree.isSuperTreeUnlinked()) {
                    out << endl << "**NOTE**:" 
                        << " Consensus tree has higher likelihood"
                        << " than ML tree found!"
                        << " Please use consensus tree below." << endl;
                }
            }
            reportTree(out, params, tree, tree.getBestScore(), 
                       tree.logl_variance, true);
...
//                if (empty_branches) {
//                    stringstream ss;
//                    ss << empty_branches << " branches in the overall tree"
//                       << " with no phylogenetic information due to missing data!";
//                    outWarning(ss.str());
//                }
                ...
                for (PhyloSuperTree::iterator it = stree->begin();
                    ...
                    //           (*it)->computeLikelihood(), 
                    //           (*it)->computeLogLVariance(), false);
                    reportTree(out, params, *(*it), 
                               stree->part_info[part].cur_score, 0.0, false);
                }
            }
        }
        /*
         if (params.write_intermediate_trees) {
         out << endl << "CONSENSUS OF INTERMEDIATE TREES"
             << endl << "-----------------------" << endl << endl
        ...
        if (params.consensus_type == CT_CONSENSUS_TREE && 
            ...
            if (params.gbo_replicates || params.num_bootstrap_samples) {
                out << endl << "Log-likelihood of consensus tree: "
                    << fixed << tree.boot_consense_logl;
            }
            ...
            if (params.compute_ml_tree)
                out << endl << "Robinson-Foulds distance between ML tree"
                    << " and consensus tree: "
            ...
            out << endl << "Branch lengths are optimized by maximum likelihood"
                << " on original alignment" << endl;
            out << "Numbers in parentheses are " << RESAMPLE_NAME
                << " supports (%)" << endl << endl;
...
//            out << endl << endl;
        ...
        if (params.terrace_analysis && params.compute_ml_tree) {
            ...
            try
            {
                ...
                if (terrace_size == 1) {
                    ...
                    out << "Terrace trees written"
                        << " (in compressed Newick format) to "
                        << filename << endl;
                }
            }
            catch (std::exception& e)
            {
                out << "ERROR: Terrace analysis using Terraphast failed: "
                    << e.what() << endl << endl;
            }
            out << endl;
            out << "For documentation, see the technical supplement" 
                << " to Biczok et al. (2018)" << endl;
            out << "https://doi.org/10.1093/bioinformatics/bty384";

            out << endl << endl;
            cout << "Done. Results are written in "
                 << params.out_prefix<<".iqtree file."<<endl;
        }
#endif
        /* evaluate user trees */
        ...
        if (!params.treeset_file.empty()) {
            evaluateTrees(params.treeset_file, params, 
                          &tree, info, distinct_trees);
            out.precision(4);
            out.setf(ios_base::fixed);

			out << endl << "USER TREES" << endl 
                << "----------" << endl << endl;
			out << "See " << params.out_prefix << ".trees"
                << " for trees with branch lengths." << endl << endl;
			if (params.topotest_replicates && info.size() > 1) {
                if (params.do_au_test && params.topotest_replicates < 10000) {
                    out << "WARNING: Too few replicates for AU test."
                        << " At least -zb 10000 for reliable results!" << endl << endl;
                }
        ...
        out << "TIME STAMP" << endl << "----------" << endl << endl
                ...
                << " seconds (" << convert_time(params.run_time) << ")" << endl
                << "Total wall-clock time used: " 
                << getRealTime() - params.start_real_time
                << " seconds (" 
...
void printAnalysisInfo(int model_df, IQTree& iqtree, 
                       Params& params) {
//    if (!params.raxmllib) {
    cout << "Model of evolution: ";
    if (iqtree.isSuperTree()) {
        cout << iqtree.getModelName()
    ...
    if (params.min_iterations > 0) {
        cout << "Tree search algorithm: "
             << (params.snni ? "Stochastic nearest neighbor interchange" : "IQPNNI") << endl;
        ...
        if (params.stop_condition == SC_REAL_TIME) {
            ...
            cout << "after " << params.unsuccess_iteration 
                 << " unsuccessful iterations" << endl;
            ...
            cout << "min " << params.min_correlation 
                 << " correlation coefficient" << endl;
        }

        if (!params.snni) {
            cout << "Number of representative leaves  : " 
                 << params.k_representative << endl;
            cout << "Probability of deleting sequences: " 
                 << iqtree.getProbDelete() << endl;
            cout << "Number of leaves to be deleted   : " 
                 << iqtree.getDelete() << endl;
            cout << "Important quartets assessed on: "
                    << ((params.iqp_assess_quartet == IQP_DISTANCE) ?
                            "Distance" : ((params.iqp_assess_quartet == IQP_PARSIMONY) ? "Parsimony" : "Bootstrap"))
                    << endl;
        }
        cout << "NNI assessed on: " 
             << ((params.nni5) ? "5 branches" : "1 branch") << endl;
    }
    cout << "Phylogenetic likelihood library: "
         << (params.pll ? "Yes" : "No") << endl;
    if (params.fixed_branch_length != BRLEN_FIX) {
        cout << "Branch length optimization method: "
            << ((iqtree.optimize_by_newton) ? "Newton" : "Brent") << endl;
    }
    cout << "Number of Newton-Raphson steps in NNI evaluation"
         << " and branch length optimization: " << NNI_MAX_NR_STEP
...
void computeMLDist ( Params& params, IQTree& iqtree,
                     double begin_wallclock_time, 
                     double begin_cpu_time) {
    double longest_dist;
    cout << "Computing ML distances based on" 
         <<" estimated model parameters..." << endl;
    ...
    cout << "Computing ML distances took "
        << (getRealTime() - begin_wallclock_time) 
        << " sec (of wall-clock time) "
        << (getCPUTime() - begin_cpu_time) 
        << " sec (of CPU time)" << endl;
    ...
    if (longest_dist > max_genetic_dist * 0.99) {
        outWarning("Some pairwise ML distances" 
                   " are too long (saturated)");
    }
}
...
void computeInitialDist(Params &params, IQTree &iqtree) {
    ...
    if (params.compute_jc_dist || params.compute_obs_dist || 
        ...
        if (longest_dist > max_genetic_dist * 0.99) {
            outWarning("Some pairwise distances"
                       " are too long (saturated)");
        }
    }
}
...
void pruneTaxa(Params &params, IQTree &iqtree, double *pattern_lh,
    ...
    if (!pruned_taxa.empty()) {
        ...
        //cout << "Log-likelihood    after reoptimizing model parameters: "
        //     << tree.curScore << endl;
...
void restoreTaxa(IQTree &iqtree, double *saved_dist_mat,
                 NodeVector &pruned_taxa, StrVector &linked_name) {
    if (!pruned_taxa.empty()) {
        ...
        iqtree.setCurScore(iqtree.optimizeAllBranches());
        //cout << "Log-likelihood "
        //     << "    after reoptimizing model parameters: "
        //     << tree.curScore << endl;
        ...
        //                                            true, params.model_eps, report_to_tree);
        //iqtree.setBestScore(opt_score);
...
void runApproximateBranchLengths(const Params &params, IQTree &iqtree) {
    if (!params.fixed_branch_length && params.leastSquareBranch) {
        ...
        string filename = params.out_prefix;
        filename += ".lstree";
        iqtree.printTree(filename.c_str(), 
                         WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH |
                         WT_SORT_TAXA | WT_NEWLINE);
        if (params.compute_likelihood) {
          cout << "Logl of tree with LS branch lengths: "
               << iqtree.getCurScore() << endl;
        }
        cout << "Tree with LS branch lengths written to " 
             << filename << endl;
        if (params.print_branch_lengths) {
            if (params.manuel_analytic_approx) {
                cout << "Applying Manuel's" 
                     << " analytic approximation.." << endl;
            ...
            out.close();
            cout << "LS Branch lengths written to " 
                 << filename << endl;
        }
        cout << "Total LS tree length: " 
             << iqtree.treeLength() << endl;
    }
    ...
    if (params.pars_branch_length) {
        ...
        if (params.compute_likelihood) {
            ...
            fixup_count = iqtree.setAllBranchLengthsFromParsimony
                          (true, parsimony_score);
        }
        ...
        iqtree.printTree(filename.c_str(), 
                         ...
                         WT_SORT_TAXA | WT_NEWLINE);
        if (params.compute_likelihood) {
            cout << "Logl of tree with MP branch lengths: "
                  << iqtree.getCurScore() << endl;
        }
        cout << "Tree with MP branch lengths written to " 
             << filename << endl;
        if (params.print_branch_lengths) {
            ofstream out;
            filename = params.out_prefix;
            ...
            iqtree.printBranchLengths(out);
            out.close();
            cout << "MP Branch lengths written to " 
                 << filename << endl;
        }
        cout << "Total MP tree length: " 
             << iqtree.treeLength() << endl;
    }
    ...
    if (params.bayes_branch_length) {
        ...
        cout << "Logl of tree with Bayesian branch lengths: "
             << iqtree.getCurScore() << endl;
        cout << "Tree with Bayesian branch lengths written to " 
             << filename << endl;
        if (params.print_branch_lengths) {
            ofstream out;
            filename = params.out_prefix;
            ...
            iqtree.printBranchLengths(out);
            out.close();
            cout << "Bayesian Branch lengths written to " 
                 << filename << endl;
        }
        cout << "Total Bayesian tree length: " 
             << iqtree.treeLength() << endl;
    }
}

void printSiteRates(IQTree &iqtree, const char *rate_file, 
                    bool bayes) {
    try {
        ofstream out;
        out.exceptions(ios::failbit | ios::badbit);
...
void printMiscInfo(Params &params, IQTree &iqtree, double *pattern_lh) {
    ...
    if (params.print_site_prob && !params.pll) {
        auto file_name = ((string)params.out_prefix + ".siteprob");
        printSiteProbCategory(file_name.c_str(), &iqtree, 
                              params.print_site_prob);
    }
    
    if (params.print_ancestral_sequence) {
        printAncestralSequences(params.out_prefix.c_str(), &iqtree,
                                params.print_ancestral_sequence);
    }
    
    if (params.print_site_state_freq != WSF_NONE &&
        !params.site_freq_file && !params.tree_freq_file) {
    ...
    if (params.fixed_branch_length == BRLEN_SCALE) {
        ...
        cout << "Scaled tree length and model parameters"
             << " printed to " << filename << endl;
    }

}

void printFinalSearchInfo(Params &params, IQTree &iqtree,
    ...
    if (iqtree.isSuperTree() && verbose_mode >= VerboseMode::VB_MAX) {
        ...
        cout << stree->evalNNIs << " NNIs evaluated from "
             << stree->totalNNIs << " all possible NNIs ( "
             << (int)floor(percent) << " %)" << endl;
    ...
    cout << "Total number of iterations: " 
         << iqtree.stop_rule.getCurIt() << endl;
    // cout << "Total number of partial likelihood"
    //      << " vector computations: " 
...
void startTreeReconstruction(Params &params, IQTree* &iqtree,
    ...
    if (params.root) {
        ...
        for (string outgroup_name : outgroup_names) {
            if (iqtree->aln->getSeqID(outgroup_name) < 0) {
                outError("Alignment does not have"
                         " specified outgroup taxon ", 
                         outgroup_name);
            }
        }
    }
...
//    iqtree.initSettings(params);
    ...
    runModelFinder(params, *iqtree, model_checkpoint);
}
...
void optimizeConTree(Params &params, IQTree *tree) {
    ...
    cout << "Log-likelihood of consensus tree: " 
         << tree->boot_consense_logl << endl;
    tree->setRootNode(params.root);
    tree->insertTaxa(tree->removed_seqs, tree->twin_seqs);
    tree->printTree(contree_file.c_str(), 
                    WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH |
                    WT_SORT_TAXA | WT_NEWLINE);
...
void runTreeReconstruction(Params &params, IQTree* &iqtree) {
    ...
    if (absent_states > 0) {
        cout << "NOTE: " << absent_states << " states (see above)"
             << " are not present and thus removed from Markov process"
             << " to prevent numerical problems" << endl;
    }
    
    //Make sure that no partial likelihood of IQ-TREE is initialized
    //when PLL is used to save memory
    if (params.pll) {
    ...
    /********************* Compute pairwise distances *******************/
    if ((params.start_tree == START_TREE_TYPE::STT_BIONJ || 
         params.iqp || params.leastSquareBranch) && !iqtree->root) {
        computeInitialDist(params, *iqtree);
    }
    ...
    if (params.online_bootstrap && params.gbo_replicates > 0) {
        cout << "Generating " << params.gbo_replicates 
             << " samples for ultrafast " << RESAMPLE_NAME 
             << " (seed: " << params.ran_seed << ")..." << endl;
    }
    ...
    if (params.compute_likelihood) {
        if (!iqtree->getModelFactory()) {
            iqtree->initializeModel(params, iqtree->aln->model_name,
                                    models_block, iqtree);
        }
    ...
    //UpperBounds analysis. Here, to analyse the initial tree
    //without any tree search or optimization
    /*
    ...
    if (!params.pll && params.compute_likelihood) {
        ...
        if (mem_required >= total_mem*0.95 && !iqtree->isSuperTree()) {
            ...
            if (params.lh_mem_save != LM_MEM_SAVE) {
                ...
                cout << "NOTE: Use -mem option if you want to"
                     << " restrict RAM usage further" << endl;
            }
        ...
        if (mem_required >= total_mem) {
            cerr << "ERROR: Your RAM is below minimum requirement of "
                 << (mem_required / 1073741824.0) << " GB RAM" << endl;
...
//#if defined __APPLE__ || defined __MACH__
        cout << "NOTE: " << (mem_required / 1048576) << " MB RAM"
             ...
             << " is required!" << endl;
//#else
        //cout << "NOTE: " << ((double) mem_size / 1000.0) / 1000
...
//#endif
        ...
        if (mem_required * max_procs > total_mem * iqtree->num_threads &&
            iqtree->num_threads > 0) {
            auto mem_req_per_core  = (double)mem_required/iqtree->num_threads;
            auto gb_req_per_core   = mem_req_per_core/1024/1024/1024;
            auto gb_avail_per_core = total_mem/max_procs/1024/1024/1024;
            outWarning("Memory required per CPU-core (" + 
                       convertDoubleToString(gb_req_per_core) +
                       " GB) is higher than your computer RAM per CPU-core (" +
                       convertIntToString(static_cast<int>(gb_avail_per_core))+
                       " GB), thus multiple runs may exceed RAM!");
        }
    }

    bool   finishedInitTree = false;
    double initEpsilon = params.min_iterations == 0
    ...
    if (!params.compute_ml_tree_only) {
        ...
        if (isRandomTree && !finishedInitTree) {
            ...
            if (params.compute_likelihood) {
                ...
                cout << "Log-likelihood of random tree: " 
                     << iqtree->getCurScore() << endl;
            } 
            else {
        ...
        if (!finishedInitTree) {
            ...
            double score      = params.compute_likelihood 
                                ? iqtree->getCurScore() : (0-parsimony_score);
    ...
    if (params.snni && 0<params.min_iterations && 
        verbose_mode >= VerboseMode::VB_MED) {
        if (params.compute_likelihood) {
    ...
    if (!params.final_model_opt && params.compute_likelihood) {
        ...
        if (iqtree->getCheckpoint()->getBool("finishedModelFinal")) {
            ...
            tree = iqtree->optimizeModelParameters(true, -1, iqtree);
...
//    assert(iqtree->root);
    ...
    if ((params.aLRT_replicates > 0 || params.localbp_replicates > 0 ||
         params.aLRT_test || params.aBayes_test) && !params.pll) {
        double mytime = getRealTime();
        params.aLRT_replicates = max(params.aLRT_replicates, 
                                     params.localbp_replicates);
        cout << endl;
        if (params.aLRT_replicates > 0) {
        ...
        if (params.localbp_replicates) {
            cout << "Testing tree branches by local-BP test with "
                 << params.localbp_replicates << " replicates..." << endl;
        }
        if (params.aLRT_test) {
        ...
        if (iqtree->isBifurcating()) {
            iqtree->testAllBranches(params.aLRT_threshold, iqtree->getCurScore(),
                                    pattern_lh, params.aLRT_replicates,
                                    params.localbp_replicates, params.aLRT_test,
                                    params.aBayes_test);
            cout << getRealTime() - mytime << " sec." << endl;
        } else {
            outWarning("Tree is multifurcating and such test is not applicable");
            params.aLRT_replicates = params.localbp_replicates = 0;
            params.aLRT_test = params.aBayes_test = 0;
        }
    }

    if (params.gbo_replicates > 0) {
        cout << "Creating " << RESAMPLE_NAME 
             << " support values..." << endl;
    ...
    if (params.gbo_replicates && params.online_bootstrap &&
        ...
        double weight_threshold = (params.split_threshold<1)
                                ? params.split_threshold
                                : (params.gbo_replicates-1.0)/params.gbo_replicates;
    ...
    if (Params::getInstance().writeDistImdTrees) {
        cout << endl;
        cout << "Recomputing the log-likelihood" 
             << " of the intermediate trees ... " << endl;
    ...
    if (params.upper_bound_NNI) {
        ...
        out_UB.open((char *) out_file_UB.c_str(), 
                    std::ofstream::out | std::ofstream::app);
        out_UB << iqtree->leafNum << "\t" << iqtree->aln->getNSite()
               << "\t" << iqtree->params->upper_bound_frac
               << "\t" << iqtree->skippedNNIub << "\t" << iqtree->totalNNIub
               << "\t" << iqtree->getBestScore() << endl;
        //iqtree->minUB << "\t" << iqtree->meanUB/iqtree->skippedNNIub
        //              << "\t" << iqtree->maxUB << endl;
        out_UB.close();
    }
...
void handleQuartetLikelihoodMapping(const Params &params, IQTree &iqtree) {
    if (params.lmap_num_quartets >= 0) {
        ...
        double elapsed = getRealTime()-lkmap_time;
        cout << "Likelihood mapping needed "
             << elapsed << " seconds" << endl << endl;
    }
}

void emptyFile(const std::string& file_name);

void doReconstructionRun(IQTree* tree, Alignment* alignment,
                         int orig_seed, int run,
                         const std::string& runtrees_name,
                         DoubleVector& runLnL, int& best_run,
                         ModelCheckpoint *model_checkpoint);

IQTree* setUpReplicateTree(IQTree* tree, Alignment* alignment);

void appendTreeToRunsFile(IQTree* iqtree, 
                          const std::string& runtrees_name);

void initializeTreeAndModelStructure(IQTree* tree);

void restoreTreeAndModelFromBestRun(IQTree* tree,
                                    const DoubleVector& runLnL, 
                                    const int& best_run);

/**********************************************************
 * MULTIPLE TREE RECONSTRUCTION
 ***********************************************************/
void runMultipleTreeReconstruction(Params &params, Alignment *alignment,
                                   IQTree *tree) {
    ...
    if (tree->getCheckpoint()->getVector("runLnL", runLnL)) {
        cout << endl << "CHECKPOINT: " << runLnL.size()
             << " independent run(s) restored" << endl;
    } else if (MPIHelper::getInstance().isMaster()) {
        emptyFile(runtrees_name);
    }
    ...
    int best_run   = 0;
    for (int run = 1; run < runLnL.size(); run++) {
    ...
    for (int run = static_cast<int>(runLnL.size()); 
         run < params.num_runs; run++) {
        doReconstructionRun(tree, alignment, orig_seed, run,
                            runtrees_name, runLnL, best_run,
                            model_checkpoint);
    }
    
    cout << endl << "---> SUMMARIZE RESULTS FROM "
         << runLnL.size() << " RUNS" << endl << endl;
    cout << "Run " << best_run+1 <<  " gave best log-likelihood: "
         << runLnL[best_run] << endl;

    initializeTreeAndModelStructure(tree);

    restoreTreeAndModelFromBestRun(tree, runLnL, best_run);
    
    // overwrite .treefile
    tree->printResultTree();

    if (MPIHelper::getInstance().isMaster()) {
        double cpu_time = (getCPUTime() - start_time);
        double elapsed  = (getRealTime() - start_real_time);
        cout << "Total CPU time for " 
             << params.num_runs << " runs: "
             << cpu_time << " seconds." << endl;
        cout << "Total wall-clock time for "
             << params.num_runs << " runs: "
             << elapsed << " seconds." << endl << endl;
    }
...
void doReconstructionRun(IQTree* tree, Alignment* alignment,
                         ...
                         DoubleVector& runLnL, int& best_run,
                         ModelCheckpoint *model_checkpoint) {
    auto run_name = "run" + convertIntToString(run+1);
    tree->getCheckpoint()->startStruct(run_name);
    
    tree->params->ran_seed = orig_seed + run*1000 
                    + MPIHelper::getInstance().getProcessID();
    
    cout << endl << "---> START RUN NUMBER " << run + 1
            << " (seed: " << tree->params->ran_seed << ")" << endl;
    
    tree->getCheckpoint()->put("seed", tree->params->ran_seed);
    
    // initialize random stream for replicating the run
    
    int *saved_randstream = randstream;
    init_random(tree->params->ran_seed);
    
    IQTree* iqtree = setUpReplicateTree(tree, alignment);
    
    runTreeReconstruction(*tree->params, iqtree);
    // read in the output tree file
    if (MPIHelper::getInstance().isMaster()) {
        appendTreeToRunsFile(iqtree, runtrees_name);
    }
    ...
    if (MPIHelper::getInstance().isMaster()) {
        if (tree->params->num_bootstrap_samples > 0 &&
            tree->params->consensus_type == CT_CONSENSUS_TREE &&
            (run == 0 || iqtree->getBestScore() > runLnL[best_run])) {
            // 2017-12-08: optimize branch lengths of consensus tree
            // now optimize branch lengths of the consensus tree
            string current_tree = iqtree->getTreeString();
            optimizeConTree(*tree->params, iqtree);
            // revert the best tree
            iqtree->readTreeString(current_tree);
            iqtree->saveCheckpoint();
        }
    ...
    if (tree->params->num_runs == 1) {
        reportPhyloAnalysis(*tree->params, *iqtree, *model_checkpoint);
    }
    delete iqtree;
...
IQTree* setUpReplicateTree(IQTree* tree, Alignment* alignment) {
    ...
    if (alignment->isSuperAlignment()){
        ...
        if (tree->params->num_mixlen > 1) {
            iqtree = new PhyloTreeMixlen(alignment, 
                                         tree->params->num_mixlen);
...
void initializeTreeAndModelStructure(IQTree* tree) {
    ...
    ModelsBlock *models_block = readModelsDefinition(*tree->params);
    tree->setParams(tree->params);
    tree->setNumThreads(tree->params->num_threads);
    if (!tree->getModelFactory()) {
        tree->initializeModel(*tree->params, tree->aln->model_name,
                              models_block, tree);
    }
...
void restoreTreeAndModelFromBestRun(IQTree* tree,
    ...
    if (tree->params->gbo_replicates && !tree->isSuperTreeUnlinked()) {
        string out_file = (string)tree->params->out_prefix + ".splits";
        auto last_split = tree->boot_splits.back();
        if (tree->params->print_splits_file) {
            last_split->saveFile(out_file.c_str(), InputType::IN_OTHER, true);
            cout << "Split supports printed to star-dot file " << out_file << endl;
        }

        if (tree->params->print_splits_nex_file) {
            out_file = (string)tree->params->out_prefix + ".splits.nex";
        ...
        if (tree->params->print_ufboot_trees) {
            tree->writeUFBootTrees(*tree->params);
        }
        ...
        string contree_file = string(tree->params->out_prefix) + ".contree";
        string current_tree = tree->getTreeString();
        tree->readTreeString(contree);
        tree->setRootNode(tree->params->root);
        tree->insertTaxa(tree->removed_seqs, tree->twin_seqs);
        tree->printTree(contree_file.c_str(), WT_BR_LEN |
...
void computeLoglFromUserInputGAMMAInvar(Params &params, IQTree &iqtree) {
    ...
    if (aiFile.good()) {
        ...
        aiFile.close();
        cout << "Computing tree logl based" 
             << " on the alpha and p_invar values in "
             << params.alpha_invar_file << " ..." <<
        endl;
    } else {
        stringstream errMsg;
    ...
    string aiResultsFileName = string(params.out_prefix) + "_"
                             + string(params.alpha_invar_file) + ".results";
    ...
    for (int i = 0; i < alphas.size(); i++) {
        ...
        iqtree.clearAllPartialLH();
        double lh = iqtree.getModelFactory()->optimizeParameters
                    (params.fixed_branch_length, false, 0.001);
        aiFileResults << lh << " " << iqtree.treeLength() << "\n";
        iqtree.restoreBranchLengths(lenvec);
    }
    aiFileResults.close();
    cout << "Results were written to: " << aiResultsFileName << endl;
    double elapsed = getRealTime() - params.start_real_time;
    cout << "Wall clock time used: " << elapsed << " sec." << endl;
}
...
void searchGAMMAInvarByRestarting(IQTree &iqtree) {
    ...
    iqtree.setCurScore(iqtree.computeLikelihood());
    cout << endl;
    cout << "Best initial alpha: " << bestAlpha
         << " / initial pinv: " << bestPInvar << " / ";
...
//    double p_invarMax = iqtree.aln->frac_const_sites;
    ...
    for (double alpha = alphaMin; alpha < alphaMax; alpha = alpha + stepSize) {
        ...
        for (double p_invar = p_invarMin; p_invar < p_invarMax;
             p_invar = p_invar + stepSize) {
            site_rates->setGammaShape(alpha);
            site_rates->setPInvar(p_invar);
            iqtree.clearAllPartialLH();
            double lh = iqtree.getModelFactory()->optimizeParameters
                        (params.fixed_branch_length, false, params.modelEps);
            //stringstream ss;
            //ss << fixed << setprecision(2) << alpha << " " << p_invar
            //   << " " << lh << " " << iqtree.treeLength();
            aiFileResults << alpha << " " << p_invar << " " << lh
                          << " " << iqtree.treeLength() << endl;
            //cout << ss.str() << endl;
...
void runStandardBootstrap(Params &params, Alignment *alignment, IQTree *tree) {
    ...
    if (tree->getCheckpoint()->get("bootSample", bootSample)) {
        cout << "CHECKPOINT: " << bootSample
             << " bootstrap analyses restored" << endl;
    } else if (MPIHelper::getInstance().isMaster()) {
    ...
    for (int sample = bootSample;
         sample < params.num_bootstrap_samples; ++sample) {
        cout << endl << "===> START " << RESAMPLE_NAME_UPPER
             << " REPLICATE NUMBER " << sample + 1 << endl << endl;
        ...
        if (alignment->isSuperAlignment()){
            ...
            if(params.partition_type != BRLEN_OPTIMIZE){
                ...
                boot_tree = new PhyloSuperTree(super_bootstrap_aln, super_tree);
            }
        } else {
            // allocate heterotachy tree if neccessary
            ModelInfoFromName model_info(alignment->model_name);
        ...
        if (params.print_boot_site_freq && MPIHelper::getInstance().isMaster()) {
            auto freq_path = ((string)params.out_prefix) + "."
                           + convertIntToString(sample)+".bootsitefreq";
            printSiteStateFreq(freq_path.c_str(), bootstrap_alignment);
            auto aln_path = ((string)params.out_prefix) + "."
                          + convertIntToString(sample)+".bootaln";
            bootstrap_alignment->printAlignment(params.aln_output_format,
                                                aln_path.c_str());
        }
...
//        }
    ...
    if (params.consensus_type == CT_CONSENSUS_TREE &&
        ...
        string root_name = (params.root) 
                           ? params.root : alignment->getSeqName(0);
    ...
    if (params.compute_ml_tree) {
        ...
        if (MPIHelper::getInstance().isMaster()) {
            ...
            cout << endl << "===> ASSIGN " << RESAMPLE_NAME_UPPER
                 << " SUPPORTS TO THE TREE FROM ORIGINAL ALIGNMENT" 
                 << endl << endl;
            MExtTree ext_tree;
    ...
    if (MPIHelper::getInstance().isMaster()) {
        double cpu_time = (getCPUTime() - start_time);
        double elapsed  = (getRealTime() - start_real_time);
        cout << "Total CPU time for " << RESAMPLE_NAME << ": "
             << cpu_time << " seconds." << endl;
        cout << "Total wall-clock time for " << RESAMPLE_NAME << ": "
             << elapsed << " seconds." << endl << endl;
        cout << "Non-parametric " << RESAMPLE_NAME
             << " results written to:" << endl;
        if (params.print_bootaln) {
            cout << RESAMPLE_NAME_I << " alignments:     "
                 << params.out_prefix << ".bootaln" << endl;
        }
        cout << RESAMPLE_NAME_I << " trees:          "
...
void doSymTest(Alignment *alignment, Params &params) {
    ...
    for (int i = 0; i < params.symtest_shuffle; i++) {
        ...
        if (i == 0) // original alignment
            alignment->doSymTest(i*num_parts, sym, marsym, 
                                 intsym, NULL, stats);
        else {
            int *rstream;
            init_random(params.ran_seed+i+1, false, &rstream);
            alignment->doSymTest(i*num_parts, sym, marsym, 
                                 intsym, rstream, stats);
    ...
    out << "# Matched-pair tests of symmetry" << endl
        ...
        << ((Params::getInstance().symtest == SYMTEST_BINOM)
    ...
    out << "#    IntSig:  Number of significant sequence pairs by test of internal symmetry" << endl
        ...
        << ((Params::getInstance().symtest == SYMTEST_BINOM)
              ? "#    IntBi:   P-value for binomial test of symmetry"
              : "#    IntPval: P-value for maximum test of internal symmetry") << endl;
    if (params.symtest_shuffle > 1) {
    ...
    out << "Name,SymSig,SymNon,"
        << ((Params::getInstance().symtest == SYMTEST_BINOM) ? "SymBi" : "SymPval")
    ...
    if (alignment->isSuperAlignment()) {
        ...
        out << alignment->name << ',' << sym[0] << ','
            << marsym[0] << ','  << intsym[0] << endl;
    }
    ...
    if (alignment->isSuperAlignment()) {
        ...
        if (!part_id.empty()) {
            ...
            if (params.aln_output_format == InputType::IN_NEXUS) {
                ...
                string aln_file = (string)params.out_prefix
                                + ((params.symtest_remove == 1)? ".good.phy" : ".bad.phy");
                alignment->printAlignment(params.aln_output_format, aln_file.c_str());
                string nexus_filename = (string)params.out_prefix
                                      + ((params.symtest_remove == 2)? ".good.nex" : ".bad.nex");
...
void runPhyloAnalysis(Params &params, Checkpoint *checkpoint) {
    ...
    if (params.partition_file) {
        ...
        alignment = createAlignment(params.aln_file, params.sequence_type,
                                    params.intype, params.model_name);

        if (params.freq_const_patterns) {
            auto orig_nsite = alignment->getNSite();
            alignment->addConstPatterns(params.freq_const_patterns);
            cout << "INFO: " << alignment->getNSite() - orig_nsite
                 << " const sites added into alignment" << endl;
        }

        // Initialize site-frequency model
        if (params.tree_freq_file) {
            if (checkpoint->getBool("finishedSiteFreqFile")) {
                auto freq_path = (string)params.out_prefix + ".sitefreq";
                alignment->readSiteStateFreq(freq_path.c_str());
                params.print_site_state_freq = WSF_NONE;
    ...
    if (params.print_aln_info) {
        ...
        cout << "Alignment sites statistics printed to "
            << site_info_file << endl;
    }
    ...
    if (params.min_branch_length <= 0.0) {
        ...
        if (!tree->isSuperTree() && tree->getAlnNSite() >= 100000) {
            ...
            auto digits = ceil(-log10(Params::getInstance().min_branch_length));
            tree->num_precision = max((int)digits+1, 6);
            cout.precision(12);
            cout << "NOTE: minimal branch length is reduced to "
                 << params.min_branch_length << " for long alignment" << endl;
            cout.precision(3);
        }
        // Increase the minimum branch length if PoMo is used.
        if (alignment->seq_type == SeqType::SEQ_POMO) {
            auto pop_size = alignment->virtual_pop_size;
            auto pop_size_squared = pop_size * pop_size;
            params.min_branch_length *= pop_size_squared;
            cout.precision(12);
            cout << "NOTE: minimal branch length is increased to "
                 << params.min_branch_length << " because PoMo infers"
    ...
    if (alignment->seq_type == SeqType::SEQ_POMO) {
        auto pop_size = alignment->virtual_pop_size;
        auto pop_size_squared = pop_size * pop_size;
        params.max_branch_length *= pop_size_squared;
        cout.precision(1);
        cout << "NOTE: maximal branch length is increased to "
             << params.max_branch_length << " because PoMo infers"
             << " number of mutations and frequency shifts" << endl;
        cout.precision(3);
    }
    ...
    if (params.constraint_tree_file) {
        ...
        if (params.num_bootstrap_samples || params.gbo_replicates) {
            cout << "INFO: Constraint tree will be applied to ML tree"
                 << " and all bootstrap trees." << endl;
        }
    }
    ...
    if (params.compute_seq_identity_along_tree) {
        ...
        if (!tree->rooted && !params.root) {
            outError("Tree is unrooted, thus"
...
//        runBootLhTest(params, alignment, *tree);
    ...
    // 2015-09-22: THIS IS STUPID: after deleting tree,
...
void runUnlinkedPhyloAnalysis(Params &params, Checkpoint *checkpoint) {
    ...
    if (!params.user_file.empty()) {
        ...
        auto partition_count = static_cast<int>(super_aln->partitions.size());
        part_trees.readTrees(params.user_file.c_str(), 
                             is_rooted, 0, partition_count);
        if (is_rooted) {
            outError("Rooted trees not allowed: ", params.user_file);
        }
        if (part_trees.size() != super_aln->partitions.size()) {
            outError("User tree file does not have" 
                     " the same number of trees as partitions");
        }
    ...
    ModelCheckpoint *model_checkpoint = new ModelCheckpoint;
    int part = 0;
    for (auto alnit = super_aln->partitions.begin();
         alnit != super_aln->partitions.end(); alnit++, part++) {
        
        checkpoint->startStruct((*alnit)->name);

        // allocate heterotachy tree if neccessary
        ModelInfoFromName model_info((*alnit)->model_name);
        IQTree *tree;
...
void assignBranchSupportNew(Params &params) {
    ...
    cout << ((tree->rooted) ? "rooted" : "un-rooted") << " tree with "
         << tree->leafNum - tree->rooted << " taxa and "
         << tree->branchNum << " branches" << endl;
    ...
    if (!params.treeset_file.empty()) {
        MTreeSet trees(params.treeset_file.c_str(), rooted,
    ...
    tree->printNexus(str, WT_BR_LEN, "See " + filename +
    ...
    << "#   sC: Number of concordant sites averaged over "
    << params.site_concordance << " quartets" << endl
    ...
    out.close();
    cout << "Concordance factors per branch and locus" 
         << " printed to " << filename << endl;
}
...
void assignBootstrapSupport(const char *input_trees, int burnin, int max_count,
    ...
    if (params && detectInputFile(input_trees) == InputType::IN_NEXUS) {
        ...
        if (boot_trees.equal_taxon_set) {
            boot_trees.convertSplits(taxname, sg, hash_ss,
                                     SW_COUNT, -1, params->support_tag);
    ...
    cout << "Creating " << RESAMPLE_NAME
         << " support values..." << endl;
    if (!sg.empty())
        mytree.createBootstrapSupport(taxname, boot_trees,
                                      hash_ss, params->support_tag);
    else {
...
void computeConsensusTree(const char *input_trees, int burnin,
                          int max_count, double cutoff,
                          double weight_threshold,
                          const char *output_tree,
    ...
    if (params && detectInputFile(input_trees) == InputType::IN_NEXUS) {
        ...
        /*        StrVector sgtaxname;
         sg.getTaxaName(sgtaxname);
         i = 0;
         for (auto sit = sgtaxname.begin(); sit != sgtaxname.end(); sit++, i++) {
         Node *leaf = mytree.findLeafName(*sit);
...
void computeConsensusNetwork(const char *input_trees, int burnin,
                             int max_count, double cutoff, int weight_summary,
                             double weight_threshold, const char *output_tree,
