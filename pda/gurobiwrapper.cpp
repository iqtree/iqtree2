...
#include <string.h>
#include <sstream>
#include <utils/stringfunctions.h> //for convert_int and convert_double
...
*/
int gurobi_solve(const char *filename, int ntaxa, double *score, double *variables, int verbosity, int num_threads) {
	int ret = 0;
	*score = -1;
	string command;
	ostringstream ss;
	...
	command = ss.str();
	if (verbosity >= static_cast<int>(VerboseMode::VB_MED))
		cout << command << endl;
