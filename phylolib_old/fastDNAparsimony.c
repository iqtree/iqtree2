...
static void testInsertParsimony(tree *tr, nodeptr p, nodeptr q,
	...
	if (tr->grouped) {
		int rNumber = tr->constraintVector[r->number], qNumber =
				tr->constraintVector[q->number], pNumber =
				tr->constraintVector[p->number];
...
static void addTraverseParsimony(tree *tr, nodeptr p, nodeptr q, int mintrav,
		int maxtrav, pl_boolean doAll, pl_boolean saveBranches) {
	if (doAll || (--mintrav <= 0))
		...
		if ((p1->number > tr->mxtips) || (p2->number > tr->mxtips)) {
			...
			if ((p1->number > tr->mxtips)) {
				addTraverseParsimony(tr, p, p1->next->back, mintrav, maxtrav,
						doAll, FALSE);
				addTraverseParsimony(tr, p, p1->next->next->back, mintrav,
						maxtrav, doAll, FALSE);
			}
			...
			if ((p2->number > tr->mxtips)) {
				addTraverseParsimony(tr, p, p2->next->back, mintrav, maxtrav,
						doAll, FALSE);
				addTraverseParsimony(tr, p, p2->next->next->back, mintrav,
						maxtrav, doAll, FALSE);
			}
		...
		if (((q1->number > tr->mxtips)
			...
			if ((q1->number > tr->mxtips)) {
				addTraverseParsimony(tr, q, q1->next->back, mintrav2, maxtrav,
						doAll, FALSE);
				addTraverseParsimony(tr, q, q1->next->next->back, mintrav2,
						maxtrav, doAll, FALSE);
			}
			...
			if ((q2->number > tr->mxtips)) {
				addTraverseParsimony(tr, q, q2->next->back, mintrav2, maxtrav,
						doAll, FALSE);
				addTraverseParsimony(tr, q, q2->next->next->back, mintrav2,
						maxtrav, doAll, FALSE);
			}
...
static pl_boolean isInformative(tree *tr, int dataType, int site) {
	int informativeCounter = 0, check[256], j, undetermined = getUndetermined(
			dataType);
...
static void compressDNA(tree *tr, int *informative) {
	...
	for (model = 0; model < (size_t) tr->NumberOfModels; model++) {
		size_t k, states = (size_t) tr->partitionData[model].states,
				compressedEntries, compressedEntriesPadded, entries = 0, lower =
						tr->partitionData[model].lower, upper =
						tr->partitionData[model].upper;

		parsimonyNumber **compressedTips = (parsimonyNumber **) malloc(
				states * sizeof(parsimonyNumber*)), *compressedValues =
				(parsimonyNumber *) malloc(states * sizeof(parsimonyNumber));
		...
		tr->partitionData[model].parsVect = (parsimonyNumber *) malloc_aligned(
		...
		for (i = 0; i < (size_t) tr->mxtips; i++) {
			...
			for (k = 0; k < states; k++) {
				compressedTips[k] =
						&(tr->partitionData[model].parsVect[(compressedEntriesPadded
								* states * (i + 1))
	...
	tr->parsimonyScore = (unsigned int*) malloc_aligned(
...
void allocateParsimonyDataStructures(tree *tr) {
	int i, *informative = (int *) malloc(
			sizeof(int) * (size_t) tr->originalCrunchedLength);
...
void makeParsimonyTreeFast(tree *tr) {
	...
	int i, nextsp, *perm = (int *) malloc(
			(size_t) (tr->mxtips + 1) * sizeof(int));
...
void parsimonySPR(nodeptr p, tree *tr) {
	...
	double p1z[NUM_BRANCHES], p2z[NUM_BRANCHES];

	nodeptr p1 = p->next->back, p2 = p->next->next->back;
