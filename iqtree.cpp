...
void IQTree::initCandidateTreeSet(int nParTrees, int nNNITrees) {
    ...
    *****************************************************************************************/
    ...
    intermediateTrees.getAllTrees(trees, scores, WT_TAXON_ID + WT_BR_LEN + WT_BR_LEN_SHORT);
...
#ifdef _IQTREE_MPI
    int nniPerProc; // Number of initial NNI search per process
    int totalNNISearches; // Total number of NNI searches
    if (Params::getInstance().numNNITrees <= MPIHelper::getInstance().getNumProcesses()) {
        nniPerProc = 1;
    } else {
        nniPerProc = Params::getInstance().numNNITrees / MPIHelper::getInstance().getNumProcesses();
    ...
    totalNNISearches = nniPerProc * MPIHelper::getInstance().getNumProcesses();
    bestTreeStrings = candidateTrees.getBestTreeStrings(totalNNISearches);

    // Dataset is too small (4, 6 taxa) so that not enough distinct initial trees can be generated
    if (candidateTrees.size() < MPIHelper::getInstance().getNumProcesses()) {
        MPI_Finalize();
...
void IQTree::selectNNIBranch(Branch curBranch, Split *curSplit, Branches &tabuBranches, SplitIntMap &tabuSplits,
    ...
    if (!candidateSplitHash.empty()) {
        ...
        if (_curSplit == NULL || _curSplit->getWeight() <= params->stableSplitThreshold) {
            nniBranches.insert(pair<int, Branch>(pairInteger(curBranch.first->id, curBranch.second->id),
                                                 curBranch));
        } else { // add a stable branch with a certain probability
            double rndDbl = random_double();
            if (rndDbl > params->stableSplitThreshold) {
                nniBranches.insert(
                        pair<int, Branch>(pairInteger(curBranch.first->id, curBranch.second->id),
                                          curBranch));
            }
        }
    }
...
Branches IQTree::getNNIBranches(SplitIntMap &tabuSplits, SplitIntMap &candidateSplitHash,Branches &nonNNIBranches, Node *node, Node *dad) {
    ...
    FOR_NEIGHBOR_IT(node, dad, it) {
            if (isInnerBranch((*it)->node, node)) {
                ...
                if (params->fixStableSplits) {
                    ...
                    if (curSplit->shouldInvert())
                        curSplit->invert();
                    selectNNIBranch(curBranch, curSplit, nonNNIBranches, tabuSplits, candidateSplitHash, nniBranches);
                    delete curSplit;
                } else {
                    nniBranches.insert(
                            pair<int, Branch>(pairInteger(curBranch.first->id, curBranch.second->id), curBranch));
            ...
            Branches _nniBranches = getNNIBranches(tabuSplits, candidateSplitHash, nonNNIBranches, (*it)->node, node);
            nniBranches.insert(_nniBranches.begin(), _nniBranches.end());
        }
    return nniBranches;
}

Branches IQTree::getStableBranches(SplitIntMap &candSplits, double supportValue, Node *dad, Node *node) {
    Branches stableBranches;
    if (!node) {
        node = root;
    }

    cout << "Number of splits = " << candSplits.size() << endl;
    ...
    FOR_NEIGHBOR_IT(node, dad, it) {
            if (isInnerBranch((*it)->node, node)) {
                ...
                if (sp != NULL) {
                    double splitSupport = (double) occurences / candSplits.getNumTree();
                    cout << "splitSupport = " << splitSupport << endl;
                    exit(0);

                    if ( splitSupport >= supportValue) {
...
string IQTree::perturbStableSplits(double supporValue) {
    Branches stableBranches = getStableBranches(candidateTrees.getCandSplits(), supporValue);
    cout << "Number of stable branches = " << stableBranches.size() << endl;
...
//    }
    ...
    if ((Params::getInstance().tabu || Params::getInstance().adaptPertubation) && candidateTrees.size() > 1) {
...
double IQTree::doTreePerturbation() {
    if (iqp_assess_quartet == IQP_BOOTSTRAP) {
        ...
        if (params->snni) {
            int nNonStableBranches =
                    (int) (aln->getNSeq() - 3 -
                           floor(candidateTrees.getNumStableSplits() * params->stableSplitThreshold));
            int nNNI = (int) floor(Params::getInstance().initPS * nNonStableBranches);
...
pair<int, int> IQTree::optimizeNNI(bool speedNNI) {
    ...
    for (numSteps = 1; numSteps <= MAXSTEPS; numSteps++) {
        ...
        if (speedNNI && !compatibleNNIs.empty()) {
            Branches filteredNNIBranches = filterNNIBranches(compatibleNNIs);
            if (tabuSplits.size() != 0 || candidateTrees.getCandSplits().size() != 0) {
                Branches nonTabuBranches = getNNIBranches(tabuSplits, candidateTrees.getCandSplits(), nonNNIBranches);
                for (Branches::iterator it = filteredNNIBranches.begin(); it != filteredNNIBranches.end(); it++) {
                    if (nonTabuBranches.find(it->first) != nonNNIBranches.end()) {
                        nniBranches.insert(*it);
                    }
                }
            } else {
                nniBranches = filteredNNIBranches;
            }
        } else {
        ...
        compatibleNNIs = getCompatibleNNIs(positiveNNIs);

        // do non-conflicting positive NNIs
        doNNIs(compatibleNNIs);
...
//            //cout << "Tree getting worse: curScore = " << curScore << " / best score = " <<  compatibleNNIs.at(0).newloglh << endl;
//            // tree cannot be worse if only 1 NNI is applied
//            assert(compatibleNNIs.size() != 1);
//            doNNIs(compatibleNNIs);
//            restoreBranchLengths(lenvec);
//            clearAllPartialLH();
//            // only do the best NNI
//            compatibleNNIs.resize(1);
//            doNNIs(compatibleNNIs);
//            curScore = optimizeAllBranches(1, params->loglh_epsilon, PLL_NEWZPERCYCLE);
...
vector<NNIMove> IQTree::getCompatibleNNIs(vector<NNIMove> &positiveNNIs) {
    vector<NNIMove> compatibleNNIs;
	for (vector<NNIMove>::iterator it1 = positiveNNIs.begin(); it1 != positiveNNIs.end(); it1++) {
		bool select = true;
