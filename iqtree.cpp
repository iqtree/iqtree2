...
//    }
	...
	if (params.p_delete != -1.0) {
		k_delete = k_delete_min = k_delete_max = ceil(
	...
	if (params.online_bootstrap && params.gbo_replicates > 0) {
		cout << "Generating " << params.gbo_replicates
				<< " samples for ultrafast bootstrap..." << endl;
		...
		BootValType *mem = aligned_alloc<BootValType>(
				nptn * (size_t) (params.gbo_replicates));
		memset(mem, 0,
				nptn * (size_t) (params.gbo_replicates) * sizeof(BootValType));
...
void IQTree::createPLLPartition(Params &params,
	...
	if (isSuperTree()) {
		...
		if (siqtree->size() > PLL_NUM_BRANCHES)
			outError(
					"Number of partitions exceeds PLL limit, please increase PLL_NUM_BRANCHES constant in pll.h");
		int i = 0;
		int startPos = 1;
		for (PhyloSuperTree::iterator it = siqtree->begin();
			...
			if ((*it)->aln->seq_type == SEQ_DNA) {
				...
				if (siqtree->part_info[i - 1].model_name != ""
						&& siqtree->part_info[i - 1].model_name.substr(0, 4)
								!= "TEST")
					pllPartitionFileHandle
							<< siqtree->part_info[i - 1].model_name.substr(0,
									siqtree->part_info[i - 1].model_name.find_first_of(
											"+{"));
				else
		...
		if (aln->seq_type == SEQ_DNA) {
			...
			if (params.pll && params.model_name != ""
				...
				model = params.model_name.substr(0,
						params.model_name.find_first_of("+{"));
			} else {
				model = "WAG";
			}
		} else {
		...
		pllPartitionFileHandle << model << ", p1 = " << "1-" << getAlnNSite()
				<< endl;
	}
}
...
void IQTree::computeInitialTree(string &dist_file) {
	...
	if (params->user_file) {
		...
		if ((params->SSE == LK_EIGEN || params->SSE == LK_EIGEN_SSE)
				&& !isBifurcating()) {
			cout
					<< "NOTE: Changing to old kernel as input tree is multifurcating"
					<< endl;
			params->SSE = LK_SSE;
		}
		if (params->pll)
		...
		case STT_PARSIMONY:
			...
			cout
					<< "Creating initial parsimony tree by random order stepwise addition..."
					<< endl;
...
void IQTree::initCandidateTreeSet(int nParTrees, int nNNITrees) {
	...
	for (vector<string>::iterator it = unOptParTrees.begin() + 1;
			it != unOptParTrees.end(); it++) {
		readTreeString(*it);
	...
	setCurIt(1);
	for (rit = initParsimonyTrees.rbegin(); rit != initParsimonyTrees.rend();
			++rit, setCurIt(getCurIt() + 1)) {
		...
		if (verbose_mode >= VB_MED) {
			cout << " / NNIs, steps: " << nniCount << "," << nniStep;
			cout << " / Parsimony logl " << initLogl << " / NNI logl: "
					<< nniLogl;
		}
		cout << " / Time: "
...
//        	((PhyloSuperTreePlen*)&iqtree)->printNNIcasesNUM();
	...
	cout << "Average CPU time for 1 NNI search: "
			<< nniTime / initParsimonyTrees.size() << endl;
	...
	if (params->fixStableSplits) {
		...
		cout << ((double) nSupportedSplits / (aln->getNSeq() - 3)) * 100
				<< " % of the splits have 100% support and can be fixed."
				<< endl;
	}
}
void IQTree::initializePLL(Params &params) {
	...
	pllQueue *partitionInfo = pllPartitionParseString(
			pllPartitionFileHandle.str().c_str());
	...
	try {
		if (!getModelFactory()) {
			if (isSuperTree()) {
				if (params.partition_type) {
					setModelFactory(
							new PartitionModelPlen(params,
									(PhyloSuperTreePlen*) this));
				} else
					setModelFactory(
...
SplitGraph IQTree::convertNNI2Splits(vector<NNIMove> nnis, int numNNI) {
	SplitGraph mySplits;
	for (int i = 0; i < numNNI; i++) {
		Split* sp = getSplit(nnis[i].node1, nnis[i].node2);
		mySplits.push_back(sp);
	}
	return mySplits;
}
...
Branches IQTree::getNonTabuBranches(Branches& branches, SplitGraph& splits) {
    assert(splits.size() > 0);
	Branches nonTabuBranches;
	for (Branches::iterator it = branches.begin(); it != branches.end(); it++) {
		assert(isInnerBranch(it->first, it->second));
...
void IQTree::doRandomNNIs(int numNNI) {
	...
	if (params->fixStableSplits) {
		tabuSplits.clear();
		int numFixedSplits = floor(
				candidateTrees.getStableSplits().size()
						* (1.0 - params->relaxStableSplits));
		SplitGraph fixedSplits = candidateTrees.getRandStableSplits(
				numFixedSplits);
		tabuSplits.assign(fixedSplits.begin(), fixedSplits.end());
	}
	int cntNNI = 0;
	while (cntNNI < numNNI) {
		...
		if (params->fixStableSplits) {
			getNonTabuBranches(innerBranches, tabuSplits);
		}
		// randomly take an inner branch and do a random NNI
		int index = random_int(innerBranches.size());
		doOneRandomNNI(innerBranches[index]);
		if (params->fixStableSplits) {
			Split* newSp = getSplit(nodes1[index], nodes2[index]);
			tabuSplits.push_back(newSp);
		}
		cntNNI++;
	}
...
void IQTree::pllPrintModelParams() {
	...
	for (int part = 0; part < pllPartitions->numberOfPartitions; part++) {
		...
		if (aln->num_states == 4) {
			...
			for (int i = 0; i < 4; i++) {
				cout
						<< pllPartitions->partitionData[part]->empiricalFrequencies[i]
						<< " ";
			}
			cout << endl;
		}
...
void IQTree::inputModelPLL2IQTree() {
	...
	if (aln->num_states == 4) {
		((ModelGTR*) getModel())->setRateMatrix(
				pllPartitions->partitionData[0]->substRates);
...
void IQTree::inputModelIQTree2PLL() {
	...
	if (aln->num_states == 4) {
		...
		if (params->pll) {
			outError(
					"Phylogenetic likelihood library current does not support data type other than DNA or Protein");
		}
	}
}
...
void IQTree::pllBaseSubstitute(char *seq, int dataType) {
	...
	meaningDNA[(int) 'N'] = meaningDNA[(int) 'n'] = meaningDNA[(int) 'O'] =
			meaningDNA[(int) 'o'] = meaningDNA[(int) 'X'] =
					meaningDNA[(int) 'x'] = meaningDNA[(int) '-'] =
							meaningDNA[(int) '?'] = 15;
...
//		}
		double modOptScore = getModelFactory()->optimizeParameters(
				params->fixed_branch_length, printInfo, epsilon);
		if (isSuperTree()) {
		...
		if (getModelFactory()->isUnstableParameters()
			...
			cout << endl;
			outWarning(
					"Estimated model parameters are at boundary that can cause numerical instability!");
			cout << endl;
		}
...
//            lhComputed = true;
		...
		curScore = pllInst->likelihood;
		pllTreeToNewick(pllInst->tree_string, pllInst, pllPartitions,
				pllInst->start->back, PLL_TRUE, PLL_TRUE, PLL_FALSE, PLL_FALSE,
				PLL_FALSE,
				PLL_SUMMARIZE_LH, PLL_FALSE, PLL_FALSE);
...
//    string perturb_tree_string;
	...
	for (; !stop_rule.meetStopCondition(curIt, cur_correlation); curIt++) {
		...
		if (params->avoid_duplicated_trees && max_candidate_trees > 0
			...
			int predicted_iteration = ((curIt + params->step_iterations - 1)
					/ params->step_iterations) * params->step_iterations;
		...
		if (iqp_assess_quartet == IQP_BOOTSTRAP) {
			...
			if (params->snni) {
				int numStableBranches = aln->getNSeq() - 3
						- candidateTrees.getStableSplits().size();
				int numNNI = floor(
						searchinfo.curPerStrength * numStableBranches);
...
//            perturb_tree_string = getTreeString();
		...
		cout
				<< ((iqp_assess_quartet == IQP_BOOTSTRAP) ?
						"Bootstrap " : "Iteration ") << curIt << " / LogL: ";
		if (verbose_mode >= VB_MED)
			cout << perturbScore << " -> ";
		cout << curScore;
		if (verbose_mode >= VB_MED)
		...
		if ((curIt) % (params->step_iterations / 2) == 0
			...
			cout << "NOTE: " << treels_logl.size()
					<< " bootstrap candidate trees evaluated (logl-cutoff: "
					<< logl_cutoff << ")" << endl;
			...
			if (curIt % params->step_iterations == 0) {
				...
				cout
						<< "NOTE: Bootstrap correlation coefficient of split occurrence frequencies: "
...
double IQTree::optimizeNNI(int &nni_count, int &nni_steps) {
    ...
    bool stopTabu = false;
    int initTabuSize = tabuSplits.size();
...
double IQTree::pllOptimizeNNI(int &totalNNICount, int &nniSteps,
	...
	if (nniSteps == (MAX_NNI_STEPS + 1)) {
		cout << "WARNING: NNI search needs unusual large number of steps ("
				<< MAX_NNI_STEPS << ") to converge!" << endl;
	}
...
void IQTree::pllInitUFBootData() {
	if (pllUFBootDataPtr == NULL) {
		...
		if (params->online_bootstrap && params->gbo_replicates > 0) {
			...
			pllUFBootDataPtr->treels_logl = (double *) malloc(
			...
			pllUFBootDataPtr->treels_newick = (char **) malloc(
			...
			pllUFBootDataPtr->treels_ptnlh = (double **) malloc(
					max_candidate_trees * (sizeof(double *)));
			...
			pllUFBootDataPtr->boot_samples = (int **) malloc(
					params->gbo_replicates * sizeof(int *));
			if (!pllUFBootDataPtr->boot_samples)
				outError("Not enough dynamic memory!");
			for (int i = 0; i < params->gbo_replicates; i++) {
				pllUFBootDataPtr->boot_samples[i] = (int *) malloc(
						pllAlignment->sequenceLength * sizeof(int));
...
//                    params->gbo_replicates, pllAlignment->sequenceLength);
			...
			pllUFBootDataPtr->boot_logl = (double *) malloc(
					params->gbo_replicates * (sizeof(double)));
			if (!pllUFBootDataPtr->boot_logl)
				outError("Not enough dynamic memory!");
			for (int i = 0; i < params->gbo_replicates; i++)
				pllUFBootDataPtr->boot_logl[i] = -DBL_MAX;

			pllUFBootDataPtr->boot_counts = (int *) malloc(
					params->gbo_replicates * (sizeof(int)));
			...
			pllUFBootDataPtr->boot_trees = (int *) malloc(
					params->gbo_replicates * (sizeof(int)));
...
vector<NNIMove> IQTree::getCompatibleNNIs(vector<NNIMove> positiveNNIs) {
...
vector<NNIMove> IQTree::evaluateNNIs(Branches branches) {
...
void IQTree::printIntermediateTree(int brtype) {
	...
	Branches innerBranches;
	getInnerBranches(innerBranches);
	vector<NNIMove> positiveNNIs = evaluateNNIs(innerBranches);
	save_all_trees = x;
}
