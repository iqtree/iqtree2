...
namespace {
    ...
    bool parseTreeName(const char* name, Params& params) {
        ...
        else if (tree_name == "SIMA")
            params.start_tree = START_TREE_TYPE::STT_SIMILARITY_MATCHING;
...
double computePValueChiSquare(double x, int df) /* x: obtained chi-square value,  df: degrees of freedom */ {
    ...
    double y    = 1;
    double a    = 0.5 * x;
    int    even = (( df & 1 ) == 0) ? 1 : 0; //1 if df is even, 0 if not
    if (df > 1) {
        y = ex(-a);
    }
    double s = (even ? y : (2.0 * Normalz(-sqrt(x))));
    if (df > 2) {
        x = 0.5 * (df - 1.0);
