...
//			}
            ...
            if (strcmp(argv[cnt], "--state-distribution") == 0) {
                cnt++;
                if (cnt >= argc || argv[cnt][0] == '-')
                    throw "Use --state-distribution <distribution_file>,<distribution_name_1>,...<distribution_name_n>";
                
                string tmp_input = argv[cnt];
                
                size_t pos = tmp_input.find(',');
                if (pos != std::string::npos) {
                    // get the path of the distribution definition file
                    params.alisim_distribution_definitions = new char[pos + 1];
                    strcpy(params.alisim_distribution_definitions, tmp_input.substr(0, pos).c_str());
                                                                    
                    // get list of state-specific distributions
                    tmp_input.erase(0, pos + 1);
                    params.alisim_state_specific_distributions = tmp_input;
                }
                else
                    throw "Use --state-distribution <distribution_file>,<distribution_name_1>,...<distribution_name_n>";
