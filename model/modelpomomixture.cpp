...

{
    ...
    ratehet = new RateGamma(num_rate_cats, 
                            Params::getInstance().gamma_shape, 
    ...
    // initialize mixture
    mixture.prop = aligned_alloc<double>(num_rate_cats);
...
//        model->total_num_subst = ratehet->getRate(m);
        mixture.models.push_back(model);
        mixture.prop[m] = ratehet->getProp(m);
    }

    //allocate memory for mixture components.
    //so that they are continuous in RAM
    mixture.initMem();
...
void ModelPoMoMixture::restoreCheckpoint() {
    // ratehet needs to be restored first, 
    // so that decomposeRateMatrix works properly
    startCheckpoint();
...
int ModelPoMoMixture::getNDim() const {
    if (opt_mode == OPT_RATEHET) {
        ...
        return super::getNDim();
    }
    else {
        return ratehet->getNDim()+super::getNDim();
    }
}

int ModelPoMoMixture::getNDimFreq() const {
    return super::getNDimFreq();
}
...
void ModelPoMoMixture::decomposeRateMatrix() {
    ...
    // trick: reverse loop to retain eigenvalues and eigenvectors
    //        of the 0th mixture class
    for (m = nmix-1; m >= 0; m--) {
        // rescale mutation_rates
        setScale(ratehet->getRate(m));
        super::decomposeRateMatrix();
        // TODO Check! TEST: copy state frequency
        super::getStateFrequency(mixture.models.at(m)->state_freq);
        // copy eigenvalues and eigenvectors
        if (m > 0) {
            memcpy(eigenvalues+m*num_states, eigenvalues, 
                   sizeof(double)*num_states);
            memcpy(eigenvectors+m*num_states_squared, eigenvectors, 
                   sizeof(double)*num_states_squared);
            memcpy(inv_eigenvectors+m*num_states_squared, inv_eigenvectors, 
                   sizeof(double)*num_states_squared);
            memcpy(inv_eigenvectors_transposed+m*num_states_squared,
                   inv_eigenvectors_transposed, 
                   sizeof(double)*num_states_squared);
        }
        // restore mutation_rate matrix
        memcpy(mutation_rate_matrix, &saved_mutation_rate_matrix[0], 
               sizeof(double)*n_alleles*n_alleles);
    }
    // // Reset scale.
    setScale(1.0);
    updatePoMoStatesAndRateMatrix();
    super::getStateFrequency(state_freq);
}
...
double ModelPoMoMixture::optimizeParameters(double gradient_epsilon,
    ...
    opt_mode = OPT_POMO;
    double score = super::optimizeParameters(gradient_epsilon,
                                                 report_to_tree);
    opt_mode = OPT_NONE;
    ...
    if (ratehet->getNDim() > 0) {
        ...
        double score_ratehet = super::optimizeParameters(gradient_epsilon,
                                                             report_to_tree);
...
int ModelPoMoMixture::get_num_states_total() {
  // We assume that all mixture model components 
...
bool ModelPoMoMixture::isUnstableParameters() const {
    ...
    if (mixture.isUnstableParameters()) {
        return true;
    }
    return false;
}
