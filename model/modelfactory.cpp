        ...
        if (freq_type == FREQ_UNKNOWN) {
            switch (tree->aln->seq_type) {
                case SEQ_BINARY:
                    ...
                    break; // default for binary: optimized frequencies
                case SEQ_PROTEIN:
                    freq_type = FREQ_USER_DEFINED;
                    break; // default for protein: frequencies of the empirical AA matrix
                case SEQ_MORPH:
                ...
                default:
                    ...
                    break; // default for DNA and others: counted frequencies from alignment
            }
        }
...
//		model_str = model_str.substr(0, posfreq);
        ...
        if (!params.site_freq_file) {
            if (model_str.substr(0, 3) == "MIX" || freq_type == FREQ_MIXTURE) {
                ...
                model = new ModelMixture(params.model_name, model_str, model_list, models_block, freq_type, freq_params,
...
//		fused_mix_rate &= model->isMixture() && site_rate->getNRate() > 1;
            ...
            delete[] rates;
            delete[] state_freq;
            cout << "Alignment is divided into " << models->size() << " partitions with " << tree->aln->getNPattern() <<
            " patterns" << endl;
...
//			outError("Invalid use of +ASC because all constant patterns are observed in the alignment");
            ...
            cout << "Ascertainment bias correction: " << unobserved_ptns.size() << " unobservable constant patterns" <<
            endl;
...
//	}
        if (posG != string::npos && posR != string::npos) {
            if (posG == posG2 && posR != posR2) {
                outWarning(
                        "Both Gamma and FreeRate models were specified, continue with Gamma model because *G has higher priority than +R");
                posR = string::npos;
            } else {
        ...
        if (posG != string::npos) {
            ...
            if (rate_str.length() > posG + 2 + end_pos && rate_str[posG + 2 + end_pos] == OPEN_BRACKET) {
                ...
                if (close_bracket == string::npos)
                    outError("Close bracket not found in ", rate_str);
                gamma_shape = convert_double(
                        rate_str.substr(posG + 3 + end_pos, close_bracket - posG - 3 - end_pos).c_str());
...
//			}
        ...
        if (rate_str.find('+') != string::npos || rate_str.find('*') != string::npos) {
            ...
            if (posI != string::npos && posG != string::npos) {
                ...
                site_rate = new RateFreeInvar(num_rate_cats, gamma_shape, freerate_params, p_invar_sites,
                                              !fused_mix_rate, tree);
