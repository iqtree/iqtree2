...
double PartitionModel::optimizeParameters(bool fixed_len, bool write_info, double logl_epsilon) {
    PhyloSuperTree *tree = (PhyloSuperTree*)site_rate->getTree();
    double tree_lh = 0.0;
    ...
    for (int part = 0; part < ntrees; part++) {
        ...
        tree_lh += tree->at(part)->getModelFactory()->optimizeParameters(fixed_len, write_info && verbose_mode >= VB_MED, logl_epsilon);
    }
