...
void PhyloSuperTreePlen::mapTrees() {
	...
	for (it = begin(); it != end(); ++it, ++part) {
        ...
        Pattern taxa_pat  = super_aln->getPattern(part);
...
//		cout<<"------------------------------------------------------"<<endl;
	...
	SuperBranch    branch(node1, node2);
	SuperNeighbor* nei1_new = branch.lookingRight();
	SuperNeighbor* nei2_new = branch.lookingLeft();
		...
		for(int part = 0; part < ntrees; part++){
			...
			if(is_nni[part]==NNI_NO_EPSILON){
				...
				part_info[part].cur_score = at(part)->computeLikelihoodBranch(nei1_new->link_neighbors[part],
						nei2_new->link_neighbors[part]->getNode(), tree_buffers);
				...
				if(!nei1_new->link_neighbors[part]){
					...
					part_info[part].cur_score = at(part)->computeLikelihoodBranch(saved_nei[0]->link_neighbors[part],
							saved_nei[1]->link_neighbors[part]->getNode(), tree_buffers);
				}
...
void PhyloSuperTreePlen::linkCheck(int part,Node* node, Node* dad, 
	                               PhyloNeighbor* saved_link_dad_nei){
	SuperBranch    branch(dad, node);
	SuperNeighbor* dad_nei  = branch.lookingRight();
	SuperNeighbor* node_nei = branch.lookingLeft();
	FOR_EACH_SUPER_NEIGHBOR(node, dad, it, nei){
		if ( nei->link_neighbors[part] == saved_link_dad_nei ) {
			nei->link_neighbors[part] = dad_nei->link_neighbors[part];
			nei->getNode()->findNeighbor(node)->link_neighbors[part] = node_nei->link_neighbors[part];
...
void PhyloSuperTreePlen::linkCheckRe(int part,Node* node, Node* dad, 
	...
	FOR_EACH_SUPER_NEIGHBOR(node, dad, it, nei){
		if ( nei->link_neighbors[part] == superDad->findNeighbor(node)->link_neighbors[part]){
			...
			nei->link_neighbors[part] = saved_link_dad_nei;
			nei->getNode()->findNeighbor(node)->link_neighbors[part] = saved_link_node_nei;
		}
	}
}
...
void PhyloSuperTreePlen::printMapInfo() {
	...
	for (iterator it = begin(); it != end(); ++it, ++part) {
		...
		for (int i = 0; i < nodes1.size(); i++) {
			PhyloNeighbor *nei1 = nodes1[i]->findNeighbor(nodes2[i])->link_neighbors[part];
			PhyloNeighbor *nei2 = nodes2[i]->findNeighbor(nodes1[i])->link_neighbors[part];
...
void PhyloSuperTreePlen::initializeAllPartialLh() {
    ...
    at(part_order[0])->tree_buffers.borrowPatternLh(tree_buffers._pattern_lh, mem_size[0]);
    
    tree_buffers.ensurePatternLhCatAllocated(total_lh_cat_size);
    at(part_order[0])->tree_buffers.borrowPatternLhCat(tree_buffers._pattern_lh_cat, lh_cat_size[0]);
    
    tree_buffers.ensureThetaAllocated(total_block_size);
    at(part_order[0])->tree_buffers.borrowTheta(tree_buffers.theta_all, block_size[0]);

    tree_buffers.ensureScaleAllAllocated(total_mem_size);
    at(part_order[0])->tree_buffers.borrowScaleAll(tree_buffers.buffer_scale_all, mem_size[0]);

    tree_buffers.ensurePartialLhAllocated(total_buffer_size);
    at(part_order[0])->tree_buffers.borrowPartialLh( tree_buffers.buffer_partial_lh, buffer_size[0]);
	...
	for (partid = 1; partid < ntrees; partid++) {
        ...
        (*it)->tree_buffers.borrowPatternLhCat((*prev_it)->tree_buffers._pattern_lh_cat + lh_cat_size[part], lh_cat_size[here]);
        (*it)->tree_buffers.theta_all        = (*prev_it)->tree_buffers.theta_all + block_size[part];
        (*it)->tree_buffers.theta_block_size = block_size[part];
...
void PhyloSuperTreePlen::initializeAllPartialLh
    ...
    if (dad!=nullptr) {
		...
		SuperNeighbor* nei_back = branch.lookingLeft();
