...
//#endif
        for (ptn = ptn_lower; ptn < ptn_upper; ptn++) {
            ...
            if (lh_max == 0.0) {
                ...
                for (c = 0; c < ncat; c++)
                    memcpy(&partial_lh_all[c*nstates], 
                           &tip_lh[aln->STATE_UNKNOWN*nstates], 
...
//#endif
		for (ptn = ptn_lower; ptn < ptn_upper; ptn++) {
            ...
            if (left->node == root) {
                ...
                state_left = (ptn < orig_ntn) 
                           ? (aln->at(ptn))[left->node->id] 
                           : model_factory->unobserved_ptns[ptn-orig_ntn];
            }
			int state_right = (ptn < orig_ntn) 
                            ? (aln->at(ptn))[right->node->id] 
                            : model_factory->unobserved_ptns[ptn-orig_ntn];
...
#pragma omp parallel for reduction(+: my_df, my_ddf, prob_const, df_const, ddf_const) private(ptn, i, c) schedule(static,1) num_threads(num_threads)
        ...
        for (int thread_id = 0; thread_id < num_threads; ++thread_id) {
            ...
            for (TraversalInfo& trav : traversal_info) {
                computePartialLikelihood(trav, ptn_lower, ptn_upper, thread_id);
            }
            for (ptn = ptn_lower; ptn < ptn_upper; ++ptn) {
