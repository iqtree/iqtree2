...
void PhyloSuperTree::printResultTree(string suffix) {
    ...
    try {
        ...
        for (iterator it = begin(); it != end(); ++it) {
            (*it)->printTree(out, WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH | 
    ...
    if (verbose_mode >= VerboseMode::VB_MED) {
        cout << "Partition trees printed to " 
		     << tree_file_name << endl;
	}
}
...
double PhyloSuperTree::computeDist(int seq1, int seq2, double initial_dist, double &var) {
    ...
    if (initial_dist == MAX_GENETIC_DIST) {
...
void PhyloSuperTree::linkBranch(int part, SuperNeighbor *nei, SuperNeighbor *dad_nei) {
	...
	nei->link_neighbors[part]     = node_part->findNeighbor(dad_part);
...
void PhyloSuperTree::mapTrees() {
	...
	for (iterator it = begin(); it != end(); ++it, ++part) {
		...
		for (i = 0; i < static_cast<int>(leafNum); ++i) {
			...
			if (id >=0) {
				part_taxa[i] = my_taxa[id];
			}
		}
		if (verbose_mode >= VerboseMode::VB_DEBUG) {
			cout << "Subtree for partition " << part << endl;
			(*it)->drawTree(cout,  WT_BR_SCALE | WT_INT_NODE | 
...
void PhyloSuperTree::computePartitionOrder() {
    ...
    if (verbose_mode >= VerboseMode::VB_MED) {
        ...
        for (int i = 0; i < ntrees; i++) {
            cout << "  charset " << at(part_order[i])->aln->name 
			     << " = " << at(part_order[i])->aln->position_spec 
				 << ";" << endl;
		}
        cout << "end;" << endl;
    }
...
PhyloSuperTree::~PhyloSuperTree()
{
	for (vector<PartitionInfo>::reverse_iterator pit = part_info.rbegin(); 
	     pit != part_info.rend(); ++pit) {
...
NNIMove PhyloSuperTree::getBestNNIForBran(PhyloNode *node1, PhyloNode *node2, NNIMove *nniMoves) {
	...
	for (int treeid = 0; treeid < ntrees; treeid++) {
		...
		at(part)->getBestNNIForBran(nei2_part->getNode(), nei1_part->getNode(), part_info[part].nniMoves);
    ...
    if (!nni_ok[0]) {
		nni_scores[0] = -DBL_MAX;
	}
    if (!nni_ok[1]) {
		nni_scores[1] = -DBL_MAX;
	}
	...
	FOR_EACH_SUPER_NEIGHBOR(node2, node1, node2_it, nei) if (nni_ok[nniid]) 
    {
        ...
        for (int part = 0; part < ntrees; part++) {
            ...
            FOR_EACH_SUPER_NEIGHBOR(node1, nullptr, nit, neiOne) {
                if (! neiOne->link_neighbors[part] ) { 
					is_nni = false; 
					break; 
				}
            }
            FOR_EACH_SUPER_NEIGHBOR(node2, nullptr, nit, neiTwo) {
                if (! neiTwo->link_neighbors[part]) { 
					is_nni = false; 
					break; 
				}
            }
...
void PhyloSuperTree::doNNI(const NNIMove &move, bool clearLH) {
	...
	for (it = begin(), part = 0; it != end(); ++it, ++part) {
		...
		PhyloNeighbor *nei1_part = nei1->link_neighbors[part];
		PhyloNeighbor *nei2_part = nei2->link_neighbors[part];
		part_move.node1 = nei2_part->getNode();
		part_move.node2 = nei1_part->getNode();
...
void PhyloSuperTree::computeBranchLengths() {
	if (verbose_mode >= VerboseMode::VB_DEBUG) {
		cout << "Assigning branch lengths" 
		     << " for full tree with weighted average..." 
			 << endl;
	}
	int part = 0, i;
    iterator it;
...
uint64_t PhyloSuperTree::getMemoryRequiredThreaded(size_t ncategory, bool full_mem) {
    ...
    uint64_t mem = 0;
	for (i = static_cast<int>(size()) - threads; 
...
int PhyloSuperTree::countEmptyBranches(PhyloNode *node, PhyloNode *dad) {
    ...
    FOR_EACH_SUPER_NEIGHBOR(node, dad, it, nei) {
    	bool isempty = true;
    	for (PhyloNeighborVec::iterator nit = nei->link_neighbors.begin(); 
			 nit != nei->link_neighbors.end(); ++nit) {
    		if ((*nit)) {
    			isempty = false;
    			break;
    		}
    	...
    	count += countEmptyBranches(nei->getNode(), node);
    }
    return count;
}
	...
	for (iterator it = begin(); it != end(); ++it, ++part) {
		if (verbose_mode >= VerboseMode::VB_MED) {
			cout << "Partition " << saln->partitions[part]->name 
			     << " " << saln->partitions[part]->getNSeq() 
				 << " sequences from " << (*it)->aln->getNSeq() 
				 << " extracted" << endl;
		}
		(*it)->aln = saln->partitions[part];
	}
	if (verbose_mode >= VerboseMode::VB_MED) {
		cout << "Reduced alignment has " << aln->getNSeq() 
		     << " sequences with " << getAlnNSite() << " sites"
			 << " and " << getAlnNPattern() << " patterns" << endl;
	}
...
void PhyloSuperTree::writeBranch(ostream &out, Node* node1, Node* node2) {
    SuperNeighbor *nei1 = dynamic_cast<SuperNeighbor*>(node1->findNeighbor(node2));
    StrVector taxnames;
...
void PhyloSuperTree::printBestPartitionParams(const char *filename) {
    try {
        ...
        for (part = 0; part < size(); part++) {
            ...
            out << "  charset " << name << " = ";
            if (!saln->partitions[part]->aln_file.empty()) {
				out << saln->partitions[part]->aln_file << ": ";
			}
