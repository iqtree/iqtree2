...
void PhyloTree::computePartialInfo(const TraversalInfo &info, VectorClass* buffer) {
    ...
    std::vector<int> cat_id_vector(ncat_mix);
    std::vector<int> mix_addr_nstates_vector(ncat_mix);
    std::vector<int> mix_addr_vector(ncat_mix);
    int* cat_id              = cat_id_vector.data();
    int* mix_addr_nstates    = mix_addr_nstates_vector.data();
    int* mix_addr            = mix_addr_vector.data();
    ...
    if (!model_to_use->useRevKernel()) {
        ...
        FOR_EACH_PHYLO_NEIGHBOR(node, dad, it, child) {
            ...
            if (child->direction == RootDirection::TOWARD_ROOT) {
                ...
                std::vector<double> mat_vector(nstatesqr);
                double* mat = mat_vector.data();
                for (int c = 0; c < ncat_mix; c++) {
                    double  cat_rate   = rate_model->getRate(c%ncat);
                    double  len_child  = cat_rate * child->length;
                    model_factory->computeTransMatrix
                        ( len_child, &mat[0], c/denom );
                ...
                for (int c = 0; c < ncat_mix; ++c) {
                    ...
                    double len_child = cat_rate * child->length;
                    model_factory->computeTransMatrix
                        ( len_child, &echild[c*nstatesqr], c/denom );
                }
            }
            ...
            if (isRootLeaf(childNode)) {
                for (int c = 0; c < ncat_mix; c++) {
                    int  m = c/denom;
                    model_to_use->getStateFrequency(partial_lh_leaf + c*nstates, m);
                }
                partial_lh_leaf += (aln->STATE_UNKNOWN+1)*block;
            } else if (childNode->isLeaf()) {
                int state_count = static_cast<int>(aln->STATE_UNKNOWN) + 1;
                if (nstates % VectorClass::size() == 0) {
                    // vectorized version
                    int nstates_on_vsize = nstates / VectorClass::size();
                    for (int state = 0; state < state_count ; ++state) {
                        VectorClass* this_tip_partial_lh  = (VectorClass*)&tip_lh[state*nstates];
                        ...
                        for (int x = 0; x < block; ++x) {
                            ...
                            for (int i = 1; i < nstates_on_vsize; ++i) {
                                vchild += echild_ptr[i] * this_tip_partial_lh[i];
                            }
                            echild_ptr += nstates_on_vsize;
                    ...
                    for (int state = 0; state < state_count; ++state) {
                        double* this_tip_partial_lh  = &tip_lh[state*nstates];
                        double* this_partial_lh_leaf = &partial_lh_leaf[state*block];
    ...
    if (nstates % VectorClass::size() == 0) {
        ...
        FOR_EACH_PHYLO_NEIGHBOR(node, dad, it, child) {
            ...
            for (int c = 0; c < ncat_mix; c++) {
                int         cat         = cat_id[c];
                double      cat_rate    = rate_model->getRate(cat);
                double      len_for_cat = child->getLength(cat);
                double      rated_len   = cat_rate * len_for_cat;
                VectorClass len_child   = static_cast<double>(rated_len);
                double*     eval_ptr    = eval + mix_addr_nstates[c];
                double*     evec_ptr    = evec + mix_addr[c];
                for (int i = 0; i < nstates/VectorClass::size(); i++) {
                    //eval_ptr is aligned (!), because mix_add_nstates[c]
                    //is divisible by nstates, and nstates is divisible
                    //by VectorClass::size().  So we *can* use load_a here.
                    expchild[i] = exp(VectorClass().load_a(&eval_ptr[i*VectorClass::size()]) * len_child);
                }
                for (int x = 0; x < nstates; ++x) {
                    for (int i = 0; i < nstates/VectorClass::size(); ++i) {
                        //evec_ptr is aligned (!), because mix_addr
                        //is divisible, not just by nstates, but by 
                        //nstates*nstates, and nstates is divisible
                        //by VectorClass::size().  
                        //So we *can* use load_a here.
            ...
            if (childNode->isLeaf()) {
                ...
                for (int state = 0; state < state_count; ++state) {
                    ...
                    for (int c = 0; c < ncat_mix; ++c) {
                        ...
                        VectorClass* this_tip_partial_lh = (VectorClass*)(tip_lh + offset);
                        for (int x = 0; x < nstates; ++x) {
                            VectorClass vchild = echild_ptr[0] * this_tip_partial_lh[0];
                            for (int i = 1; i < nstates/VectorClass::size(); ++i) {
                ...
                partial_lh_leaf += state_count * block;
            }
            echild += block*nstates;
        }
    } else {
        // non-vectorized version
        std::vector<double> expchild_vector(nstates);
        double* expchild = expchild_vector.data();
        FOR_EACH_PHYLO_NEIGHBOR(node, dad, it, child) {
            // precompute information buffer
            double *echild_ptr = echild;
            for (int c = 0; c < ncat_mix; ++c) {
                ...
                double  len_for_cat = child->getLength(cat);
                double  rated_len   = cat_rate * len_for_cat;
                double  len_child   = static_cast<double>(rated_len);
                double* eval_ptr    = eval + mix_addr_nstates[c];
                double* evec_ptr    = evec + mix_addr[c];
                for (int i = 0; i < nstates; ++i) {
...
void PhyloTree::computeTraversalInfo(PhyloNode* node, PhyloNode* dad,
    ...
    bool   fused    = model_factory->fused_mix_rate;
    auto rate_model = getRateModelForBranch(dad, node, other_rate_model);
    size_t n_rate   = rate_model->getNRate();
    size_t ncat_mix = fused ? n_rate : n_rate*model->getNMixtures();
    size_t block    = nstates * ncat_mix;
    ...
    if (!model_to_use->isSiteSpecificModel()) {
...
#pragma omp for schedule(static)
    ...
    if (compute_partial_lh) {
        ...
        intptr_t orig_nptn = roundUpToMultiple(aln->size(), VectorClass::size());
        intptr_t nptn      = roundUpToMultiple(orig_nptn+model_factory->unobserved_ptns.size(),VectorClass::size());
        ...
        for (int packet_id = 0; packet_id < num_packets; ++packet_id) {
            for (TraversalInfo& trav : traversal_info) {
                computePartialLikelihood(trav, limits[packet_id], limits[packet_id+1], 
                                         packet_id, buffers);
            }
        }
        traversal_info.clear();
...
#endif
{
    ...
    size_t* mix_addr_nstates = mix_addr_nstates_vector.data();
    ...
    // precomputed buffer to save time
    const size_t thread_buf_size  = (2*block+nstates)*VectorClass::size();
    ...
    std::vector<double> len_vector(ncat*(node->degree()-1));
    double* len_children = len_vector.data();
    ...
    if (SITE_MODEL) {
        FOR_NEIGHBOR_IT(node, dad, it3) {
            for (int c = 0; c < ncat; c++) {
                len_children[c] = rate_model->getRate(c) * (*it3)->length;
            }
            if (!len_left) {
                len_left  = len_children;
            } else {
                len_right = len_children;
            }
            len_children += ncat;
        }
    } else {
    ...
    if (node->degree() > 3) {
        ...
        VectorClass* partial_lh_all = (VectorClass*) ( buffer_partial_lh_ptr 
                                    + thread_buf_size * packet_id );
        ...
        for (intptr_t ptn = ptn_lower; ptn < ptn_upper; ptn+=VectorClass::size()) {
            ...
            double*            len_child = len_vector.data();
            VectorClass        vchild;
            ...
            FOR_EACH_PHYLO_NEIGHBOR(node, dad, it, child) {
                ...
                if (SITE_MODEL) {
                    ...
                    if (childNode->isLeaf()) {
                        // external node
                        const VectorClass* tip_partial_lh_child = (VectorClass*) &tip_lh[childNode->id*tip_mem_size + ptn*nstates];
                    ...
                    UBYTE* scale_child = SAFE_NUMERIC 
                                       ? child->scale_num + ptn*ncat_mix 
                                       : nullptr;
                    if (childNode->isLeaf()) {
                        auto stateRow = getConvertedSequenceByNumber(childNode->id);
                        // external node
                        // load data for tip
            ...
            VectorClass   lh_max         = 0.0;
            const double* inv_evec_ptr = SITE_MODEL ? &inv_evec[ptn*states_square] : nullptr;
            for (size_t c = 0; c < ncat_mix; c++) {
                if (SITE_MODEL) {
                    // compute dot-product with inv_eigenvector
#ifdef KERNEL_FIX_STATES
        ...
        const double* partial_lh_left  = SITE_MODEL
            ? &tip_lh[leftNode->id  * tip_mem_size]
            : partial_lh_leaves;
        const double* partial_lh_right = SITE_MODEL
            ? &tip_lh[rightNode->id * tip_mem_size]
        ...
        for (intptr_t ptn = ptn_lower; ptn < ptn_upper; ptn+=VectorClass::size()) {
            ...
            if (SITE_MODEL) {
                ...
                for (size_t c = 0; c < ncat_mix; c++) {
                    const double *inv_evec_ptr = inv_evec + mix_addr[c];
                    // compute real partial likelihood vector
                    for (size_t x = 0; x < nstates; x++) {
                ...
                for (size_t c = 0; c < ncat_mix; c++) {
                    ...
                    if (SAFE_NUMERIC) {
                        ...
                        if (horizontal_or(underflown)) { 
                            // at least one site has numerical underflown
                            for (int x = 0; x < VectorClass::size(); x++) {
                                if (underflown[x]) {
                                    // BQM 2016-05-03: only scale for non-constant sites
                                    // now do the likelihood scaling
            ...
            for (size_t c = 0; c < ncat_mix; ++c) {
                ...
                if (SAFE_NUMERIC) {
                    auto underflown = ((lh_max < SCALING_THRESHOLD) 
                                    & (VectorClass().load_a(&ptn_invar[ptn]) == 0.0));
                    if (horizontal_or(underflown)) {
                        for (int x = 0, y=0; x < VectorClass::size(); x++, y+=ncat_mix) {
                            if (underflown[x]) {
                                // BQM 2016-05-03: only scale for non-constant sites
                                // now do the likelihood scaling
...
#ifdef KERNEL_FIX_STATES
template <class VectorClass, const bool SAFE_NUMERIC, const int nstates, const bool FMA, const bool SITE_MODEL>
void PhyloTree::computeLikelihoodBufferSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad
        , intptr_t ptn_lower, intptr_t ptn_upper, int packet_id
        , const LikelihoodBufferSet& buffers)
#else
template <class VectorClass, const bool SAFE_NUMERIC, const bool FMA, const bool SITE_MODEL>
void PhyloTree::computeLikelihoodBufferGenericSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad
        , intptr_t ptn_lower, intptr_t ptn_upper, int packet_id
        , const LikelihoodBufferSet& buffers)
#endif
{
    ...
    intptr_t unobs_ptn     = model_factory->unobserved_ptns.size();
    intptr_t nptn          = max_orig_nptn + unobs_ptn;
    ...
    intptr_t tip_block = nstates * n_mix;
    ...
    double *buffer_partial_lh_ptr = buffers.buffer_partial_lh 
                                  + 3 * get_safe_upper_limit(block);
    if (isMixlen()) {
        size_t nmix = getMixlen();
        buffer_partial_lh_ptr += nmix*(nmix+1)*VectorClass::size() 
                               + (nmix+3)*nmix*VectorClass::size()*num_packets;
    }
    ...
    if (dad->isLeaf()) {
        ...
        double*  tip_partial_lh_node = &tip_lh[dad->id * max_orig_nptn * nstates];
        ...
        auto     unknown    = aln->STATE_UNKNOWN;
        intptr_t offset     = ptn_lower*block;
        intptr_t offsetStep = block*VectorClass::size();
        for (intptr_t ptn = ptn_lower; ptn < ptn_upper; ptn+=VectorClass::size(), offset+=offsetStep) {
...
//            aligned_free(vec_tip);
        ...
        // now compute theta
        for (intptr_t ptn = ptn_lower; ptn < ptn_upper; ptn+=VectorClass::size()) {
            intptr_t ptn_by_block = ptn * block;
            ...
            if (SAFE_NUMERIC) {
                // numerical scaling per category
                UBYTE  min_scale;
                std::vector<UBYTE> sum_scale_vector(ncat_mix);
                UBYTE*       sum_scale  = sum_scale_vector.data();
                ...
                for (size_t i = 0; i < VectorClass::size(); i++) {
                    ...
                    for (size_t c = 1; c < ncat_mix; c++) {
                        ...
                        min_scale = (min_scale < sum_scale[c]) ? min_scale : sum_scale[c];
                    }
                    buffers.buffer_scale_all[ptn+i] = min_scale;
...
#endif
{
    ...
    ModelSubst* model_to_use = getModelForBranch(dad, node, other_model);
    RateHeterogeneity* other_rate = nullptr;
    auto        rate_model   = getRateModelForBranch(dad, node, other_rate);
    ...
    std::vector<size_t> mix_addr_nstates_vector(ncat_mix);
    std::vector<size_t> mix_addr_vector(ncat_mix);
    std::vector<size_t> cat_id_vector(ncat_mix);
    size_t* mix_addr_nstates = mix_addr_nstates_vector.data();
    size_t* mix_addr         = mix_addr_vector.data();
    size_t* cat_id           = cat_id_vector.data(); 
    int     denom            = fused ? 1 : ncat;
    double* eval             = model_to_use->getEigenvalues();
    ASSERT(eval);
    const double* ptn_invar  = model_to_use->getPatternInvar();
    ...
    std::vector<double> cat_rate_vector(ncat);
    std::vector<double> cat_prop_vector(ncat);
    double* cat_rate = cat_rate_vector.data();
    double* cat_prop = cat_prop_vector.data();

    if (SITE_MODEL) {
        for (int c = 0; c < ncat; c++) {
            cat_rate[c] = rate_model->getRate(c);
            cat_prop[c] = rate_model->getProp(c);
        }
    } else {
        ...
        if (nstates % VectorClass::size() == 0) {
            ...
            for (int c = 0; c < ncat_mix; c++) {
                ...
                double    myrate   = rate_model->getRate(mycat);
...
#pragma omp parallel for schedule(dynamic,1) num_threads(num_threads) reduction(+:all_lh,all_df,all_ddf,all_prob_const,all_df_const,all_ddf_const)
    ...
    *ddf = all_ddf;
    ...
    if (ASC_Holder) {
        ...
        // cutoff the last entries if going beyond
        for (intptr_t ptn = orig_nptn; ptn < max_orig_nptn; ptn++) {
            const_lh[ptn]  = 0.0;
            const_df[ptn]  = 0.0;
            const_ddf[ptn] = 0.0;
        }
        VectorClass sum_df = 0.0;
        VectorClass sum_ddf = 0.0;
...
#ifdef KERNEL_FIX_STATES
template <class VectorClass, const bool SAFE_NUMERIC, const int nstates, const bool FMA, const bool SITE_MODEL>
double PhyloTree::computeLikelihoodBranchSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad,
         LikelihoodBufferSet& buffers)
#else
template <class VectorClass, const bool SAFE_NUMERIC, const bool FMA, const bool SITE_MODEL>
double PhyloTree::computeLikelihoodBranchGenericSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad,
         LikelihoodBufferSet& buffers)
#endif
{
    ...
    if (SITE_MODEL) {
        for (int c = 0; c < ncat; c++) {
            cat_length[c] = rate_model->getRate(c) * dad_branch->length;
            cat_prop[c]   = rate_model->getProp(c);
        }
    } else {
        ...
        if (nstates % VectorClass::size() == 0) {
            ...
            for (int c = 0; c < ncat_mix; c++) {
                ...
                double len            = rate_model->getRate(mycat)
                                      * dad_branch->getLength(mycat);
                double prop           = rate_model->getProp(mycat) 
                                      * model_to_use->getMixtureWeight(m);
    ...
    if (dad->isLeaf()) {        
        ...
        if (SITE_MODEL) {
            partial_lh_node = &tip_lh[dad->id * tip_mem_size];
        }
        else {
...
//            states_dad.push_back(aln->STATE_UNKNOWN);
            ...
            if (nstates % VectorClass::size() == 0) {
                ...
                for (int state = 0; state <= static_cast<int>(aln->STATE_UNKNOWN); ++state) {
                    ...
                    const double* lh_tip     = tip_lh          + state * tip_block;
                ...
                for (int state = 0; state <= static_cast<int>(aln->STATE_UNKNOWN); state++) {
                    ...
                    const double* this_tip_partial_lh = tip_lh + state * tip_block;
                    for (size_t c = 0; c < ncat_mix; c++) {
        ...
        for (int packet_id = 0; packet_id < num_packets; packet_id++) {
            ...
            for (intptr_t ptn = ptn_lower; ptn < ptn_upper; ptn+=VectorClass::size()) {
                ...
                if (SITE_MODEL) {
                    ...
                    //load tip vector
                    for (intptr_t i = 0; i < VectorClass::size(); i++) {
                        int state;
                        if (ptn+i < orig_nptn) {
                            if (stateRow!=nullptr) {
                                state =  stateRow[ptn+i];
...
#pragma omp parallel for schedule(dynamic,1) num_threads(num_threads) reduction(+:all_tree_lh,all_prob_const)
        ...
        for (int packet_id = 0; packet_id < num_packets; packet_id++) {
            ...
            // reset memory for _pattern_lh_cat
            memset(buffers._pattern_lh_cat + ptn_lower*ncat_mix, 0, 
                   sizeof(double)*ptn_count*ncat_mix);

            // first compute partial_lh
            for (TraversalInfo& info : traversal_info) {
                computePartialLikelihood(info, ptn_lower, ptn_upper, 
                                         packet_id, buffers);
            }
...
#endif
{
    ...
    int      nstates   = aln->num_states;
#endif
    ModelSubst* model_to_use = getModelForCurrentBranch();
    ...
    #endif
    for (intptr_t ptn = 0; ptn < nptn; ptn+=VectorClass::size()) {
        VectorClass lh_ptn;
        const VectorClass* theta = (VectorClass*)(buffers.theta_all + ptn*block);
        if (SITE_MODEL) {
            VectorClass* eval_ptr = (VectorClass*)&eval[ptn*nstates];
...
#ifdef KERNEL_FIX_STATES
template <class VectorClass, const bool SAFE_NUMERIC, const int nstates, const bool FMA, const bool SITE_MODEL>
void PhyloTree::computeLikelihoodDervMixlenSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad,
         double &df, double &ddf, const LikelihoodBufferSet& buffers)
#else
template <class VectorClass, const bool SAFE_NUMERIC, const bool FMA, const bool SITE_MODEL>
void PhyloTree::computeLikelihoodDervMixlenGenericSIMD
        (PhyloNeighbor* dad_branch, PhyloNode* dad,
         double& df, double& ddf, const LikelihoodBufferSet& buffers)
#endif
{
    ...
    bool     fused         = model_factory->fused_mix_rate;
    size_t   n_mix         = model->getNMixtures();
    size_t   ncat_mix      = fused ? ncat : ncat * n_mix;
    size_t   nmix          = fused ? 1 : n_mix;
    size_t   block         = ncat_mix * nstates;
    intptr_t orig_nptn     = aln->size();
...
//    }
    ...
    if (SITE_MODEL) {
        ...
        for (int c = 0; c < nmix; c++) {
            bool    fused    = model_factory->fused_mix_rate;
            int     cur_mix  = fused ? cur_mixlen : c;
            double* eval_ptr = eval+cur_mix*nstates;
            double  prop     = model_to_use->getMixtureWeight(cur_mix);
            int     addr     = c*nstates;
            for (size_t i = 0; i < nstates; i++) {
...
#pragma omp parallel for schedule(dynamic,1) num_threads(num_threads) reduction(+:all_df,all_ddf,all_prob_const,all_df_const,all_ddf_const)
	...
	if (ASC_Lewis) {
        ...
        double nsites = static_cast<double>(aln->getNSite());
