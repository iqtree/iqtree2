...
void initializePartitionData(pllInstance *localTree, partitionList * localPartitions) {
	...
	size_t maxCategories = (size_t) localTree->maxCategories;

	size_t ancestralVectorWidth = 0, model;

	int tid = localTree->threadID, innerNodes = localTree->mxtips - 2;
	...
	for (model = 0; model < (size_t) localPartitions->numberOfPartitions; model++) {
		size_t width = localPartitions->partitionData[model]->width;

		const partitionLengths *pl = getPartitionLengths(localPartitions->partitionData[model]);
		...
		localPartitions->partitionData[model]->globalScaler = (unsigned int *) rax_calloc(
				2 * (size_t) localTree->mxtips, sizeof(unsigned int));

		localPartitions->partitionData[model]->left =
				(double *) rax_malloc_aligned((size_t)pl->leftLength * (maxCategories + 1) * sizeof(double));
		localPartitions->partitionData[model]->right =
				(double *) rax_malloc_aligned((size_t)pl->rightLength * (maxCategories + 1) * sizeof(double));
		...
		localPartitions->partitionData[model]->substRates = (double *) rax_malloc(
				(size_t) pl->substRatesLength * sizeof(double));
		localPartitions->partitionData[model]->frequencies = (double*) rax_malloc(
				(size_t) pl->frequenciesLength * sizeof(double));
		localPartitions->partitionData[model]->freqExponents = (double*) rax_malloc(
				pl->frequenciesLength * sizeof(double));
		localPartitions->partitionData[model]->empiricalFrequencies = (double*) rax_malloc(
				(size_t) pl->frequenciesLength * sizeof(double));
		localPartitions->partitionData[model]->tipVector =
				(double *) rax_malloc_aligned((size_t)pl->tipVectorLength * sizeof(double));
		...
		if (localPartitions->partitionData[model]->dataType == PLL_AA_DATA
				&& localPartitions->partitionData[model]->protModels == PLL_LG4) {
			int k;

			for (k = 0; k < 4; k++) {
				localPartitions->partitionData[model]->EIGN_LG4[k] = (double*) rax_malloc(
						pl->eignLength * sizeof(double));
				localPartitions->partitionData[model]->EV_LG4[k] =
						(double*) rax_malloc_aligned(pl->evLength * sizeof(double));
				localPartitions->partitionData[model]->EI_LG4[k] = (double*) rax_malloc(pl->eiLength * sizeof(double));
				localPartitions->partitionData[model]->substRates_LG4[k] = (double *) rax_malloc(
						pl->substRatesLength * sizeof(double));
				localPartitions->partitionData[model]->frequencies_LG4[k] = (double*) rax_malloc(
						pl->frequenciesLength * sizeof(double));
				localPartitions->partitionData[model]->tipVector_LG4[k] =
						(double *) rax_malloc_aligned(pl->tipVectorLength * sizeof(double));
			}
		}

		localPartitions->partitionData[model]->symmetryVector = (int *) rax_malloc(
				(size_t) pl->symmetryVectorLength * sizeof(int));
		localPartitions->partitionData[model]->frequencyGrouping = (int *) rax_malloc(
				(size_t) pl->frequencyGroupingLength * sizeof(int));
		...
		localPartitions->partitionData[model]->yVector = (unsigned char **) rax_malloc(
				sizeof(unsigned char*) * ((size_t) localTree->mxtips + 1));

		localPartitions->partitionData[model]->xVector = (double **) rax_calloc(sizeof(double*),
				(size_t) localTree->mxtips);
		...
		localPartitions->partitionData[model]->xSpaceVector = (size_t *) rax_calloc((size_t) localTree->mxtips,
				sizeof(size_t));
		...
		localPartitions->partitionData[model]->perSiteLikelihoods =
				(double *) rax_malloc_aligned( width * sizeof(double));
		...
		ancestralVectorWidth += ((size_t) (localPartitions->partitionData[model]->upper
				- localPartitions->partitionData[model]->lower)
				* (size_t) (localPartitions->partitionData[model]->states) * sizeof(double));
		...
		if (width > 0 && localTree->saveMemory) {
			...
			localPartitions->partitionData[model]->gapVector = (unsigned int*) rax_calloc(
					(size_t) localPartitions->partitionData[model]->gapVectorLength * 2 * (size_t) localTree->mxtips,
					sizeof(unsigned int));
			localPartitions->partitionData[model]->gapColumn =
					(double *) rax_malloc_aligned(((size_t)localTree->mxtips) *
							((size_t)(localPartitions->partitionData[model]->states)) *
...
void initMemorySavingAndRecom(pllInstance *tr, partitionList *pr) {
	pllInstance *localTree = tr;
	partitionList *localPartitions = pr;
	size_t model;
	...
	if (localTree->saveMemory) {
		for (model = 0; model < (size_t) localPartitions->numberOfPartitions; model++) {
			int undetermined = getUndetermined(localPartitions->partitionData[model]->dataType);

			size_t i, j, width = localPartitions->partitionData[model]->width;

			if (width > 0) {
				for (j = 1; j <= (size_t) (localTree->mxtips); j++)
					for (i = 0; i < width; i++)
						if (localPartitions->partitionData[model]->yVector[j][i] == undetermined)
							localPartitions->partitionData[model]->gapVector[localPartitions->partitionData[model]->gapVectorLength
									* j + i / 32] |= mask32[i % 32];
			}
		}
	}
 ...
 */
static inline void swapSite(unsigned char ** buf, int s1, int s2, int nTaxa) {
	int i;
	int x;
...
/** @brief Copy a site to another buffer
 ...
 Number of taxa, i.e. size of site
 */
static inline void copySite(unsigned char ** dst, unsigned char ** src, int to, int from, int nTaxa) {
	int i;
...
void pllAlignmentRemoveDups(pllAlignmentData * alignmentData, partitionList * pl) {
	...
	alignmentData->sequenceData[0] = (unsigned char *) rax_malloc(
			(alignmentData->sequenceLength + 1) * sizeof(unsigned char) * alignmentData->sequenceCount);
	for (i = 0; i < alignmentData->sequenceCount; ++i) {
		alignmentData->sequenceData[i + 1] = (unsigned char *) (alignmentData->sequenceData[0]
				+ i * (alignmentData->sequenceLength + 1) * sizeof(unsigned char));
 ...
 */
static void genericBaseFrequencies(pInfo * partition, pllAlignmentData * alignmentData, pll_boolean smoothFrequencies,
		const unsigned int * bitMask, double * pfreqs) {
	double wj, acc, sumf[64], temp[64];

	int i, j, k, l, numFreqs, lower, upper;
...
pllBaseFrequenciesGTR(partitionList * pl, pllAlignmentData * alignmentData) {
	...
	double **freqs = (double **) rax_malloc(pl->numberOfPartitions * sizeof(double *));
	...
	for (model = 0; model < pl->numberOfPartitions; ++model) {
		...
		case PLL_AA_DATA:
		case PLL_DNA_DATA:
			genericBaseFrequencies(pl->partitionData[model], alignmentData,
					pLengths[pl->partitionData[model]->dataType].smoothFrequencies,
					pLengths[pl->partitionData[model]->dataType].bitVector, freqs[model]);
			break;
		default: {
...
int pllLoadAlignment(pllInstance * tr, pllAlignmentData * alignmentData, partitionList * partitions, int bDeep) {
	...
	if (tr->mxtips != alignmentData->sequenceCount)
		return (0);
	...
	if (bDeep == PLL_DEEP_COPY) {
		tr->yVector[0] = (unsigned char *) rax_malloc(
				sizeof(unsigned char) * (alignmentData->sequenceLength + 1) * alignmentData->sequenceCount);
		for (i = 1; i <= alignmentData->sequenceCount; ++i) {
			tr->yVector[i] = (unsigned char *) (tr->yVector[0]
					+ (i - 1) * (alignmentData->sequenceLength + 1) * sizeof(unsigned char));
...
pllCreateInstance(pllInstanceAttr * attr) {
	...
	if (attr->rateHetModel != PLL_GAMMA && attr->rateHetModel != PLL_CAT)
		return NULL;
...
static void pllTreeInitDefaults(pllInstance * tr, int tips) {
	...
	for (i = 0; i < PLL_NUM_BRANCHES; ++i)
		tr->td[0].executeModel[i] = PLL_TRUE;
}
...
void pllTreeInitTopologyForAlignment(pllInstance * tr, pllAlignmentData * alignmentData) {
	int tips = alignmentData->sequenceCount, i;
...
void pllBaseSubstitute(pllAlignmentData * alignmentData, partitionList * partitions) {
	...
	meaningDNA['N'] = meaningDNA['n'] = meaningDNA['O'] = meaningDNA['o'] = meaningDNA['X'] = meaningDNA['x'] =
			meaningDNA['-'] = meaningDNA['?'] = 15;
	...
	meaningAA['X'] = meaningAA['x'] = meaningAA['?'] = meaningAA['*'] = meaningAA['-'] = 22;
...
static int init_Q_MatrixSymmetries(char *linkageString, partitionList * pr, int model) {
	int states = pr->partitionData[model]->states, numberOfRates = ((states * states - states) / 2), *list =
			(int *) rax_malloc(sizeof(int) * numberOfRates), j, max = -1;

	char *str1, *saveptr, *ch, *token;
...
static int checkLinkageConsistency(partitionList *pr) {
	if (pr->dirty) {
		...
		for (i = 0; i < ll->entries; i++) {
			int partitions = ll->ld[i].partitions, reference = ll->ld[i].partitionList[0];
			...
			if (partitions > 1) {
				...
				for (k = 1; k < partitions; k++) {
					...
					int states = pr->partitionData[index]->states, rates = ((states * states - states) / 2);

					if (!(pr->partitionData[reference]->nonGTR == pr->partitionData[index]->nonGTR)) {
						errno = PLL_INCONSISTENT_SUBST_RATE_OPTIMIZATION_SETTING;
						return PLL_FALSE;
					}
					if (!(pr->partitionData[reference]->optimizeSubstitutionRates
							== pr->partitionData[index]->optimizeSubstitutionRates)) {
						errno = PLL_INCONSISTENT_SUBST_RATE_OPTIMIZATION_SETTING;
						return PLL_FALSE;
					}
					...
					if (pr->partitionData[reference]->nonGTR) {
						...
						for (j = 0; j < rates; j++) {
							if (!(pr->partitionData[reference]->symmetryVector[j]
									== pr->partitionData[index]->symmetryVector[j])) {
		...
		for (i = 0; i < ll->entries; i++) {
			int partitions = ll->ld[i].partitions;

			if (partitions > 1) {
				int k, reference = ll->ld[i].partitionList[0];

				for (k = 1; k < partitions; k++) {
					int index = ll->ld[i].partitionList[k];

					if (!(pr->partitionData[reference]->optimizeAlphaParameter
							== pr->partitionData[index]->optimizeAlphaParameter)) {
		...
		for (i = 0; i < ll->entries; i++) {
			...
			if (partitions > 1) {
				...
				for (k = 1; k < partitions; k++) {
					int j, index = ll->ld[i].partitionList[k], states = pr->partitionData[index]->states;

					if (!(pr->partitionData[reference]->optimizeBaseFrequencies
							== pr->partitionData[index]->optimizeBaseFrequencies)) {
...
int pllSetSubstitutionRateMatrixSymmetries(char *string, partitionList * pr, int model) {
	int result = init_Q_MatrixSymmetries(string, pr, model);
...
int pllSetOptimizeBaseFrequencies(int model, partitionList * pr, pllInstance *tr) {
	...
	double initialFrequency, acc = 0.0;
...
void pllSetFixedSubstitutionMatrix(double *q, int length, int model, partitionList * pr, pllInstance *tr) {
	...
	numberOfRates = (pr->partitionData[model]->states * pr->partitionData[model]->states
			- pr->partitionData[model]->states) / 2;
...
linkageList* initLinkageList(int *linkList, partitionList *pr) {
	int k, partitions, numberOfModels = 0, i, pos;
...
static linkageList* initLinkageListString(char *linkageString, partitionList * pr) {
	int *list = (int*) rax_malloc(sizeof(int) * pr->numberOfPartitions), j;
...
int pllInitModel(pllInstance * tr, partitionList * partitions, pllAlignmentData * alignmentData) {
	double ** ef;
	int i, *unlinked = (int *) rax_malloc(sizeof(int) * partitions->numberOfPartitions);
