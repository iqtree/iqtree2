...
static void evaluateChange(tree *tr, int rateNumber, double *value,
		double *result, boolean* converged, int whichFunction,
...
static void brentGeneric(double *ax, double *bx, double *cx, double *fb,
		...
		int whichFunction, int rateNumber, tree *tr, linkageList *ll,
	...
	for (iter = 1; iter <= ITMAX; iter++) {
		...
		evaluateChange(tr, rateNumber, u, fu, converged, whichFunction,
				numberOfModels, ll);
...
static int brakGeneric(double *param, double *ax, double *bx, double *cx,
		...
		int numberOfModels, int rateNumber, int whichFunction, tree *tr,
		linkageList *ll) {
	...
	evaluateChange(tr, rateNumber, param, fa, converged, whichFunction,
			numberOfModels, ll);
	...
	evaluateChange(tr, rateNumber, param, fb, converged, whichFunction,
			numberOfModels, ll);
	...
	evaluateChange(tr, rateNumber, param, fc, converged, whichFunction,
			numberOfModels, ll);
	...
	while (1) {
		...
		evaluateChange(tr, rateNumber, param, temp, converged, whichFunction,
				numberOfModels, ll);
...
static void optRates(tree *tr, double modelEpsilon, linkageList *ll,
		int numberOfModels, int states) {
	...
	for (i = 0; i < numberOfRates; i++) {
		...
		brentGeneric(_a, _b, _c, _fb, modelEpsilon, _x, result, numberOfModels,
				RATE_F, i, tr, ll, lim_inf, lim_sup);
...
static void categorizePartition(tree *tr, rateCategorize *rc, int model,
		int lower, int upper) {
	int zeroCounter, i, k;
...
void updatePerSiteRates(tree *tr, boolean scaleRates) {
	...
	if (tr->numBranches > 1) {
		for (model = 0; model < tr->NumberOfModels; model++) {
			...
			if (scaleRates) {
				...
				for (i = lower; i < upper; i++) {
					...
					double rate =
							tr->partitionData[model].perSiteRates[tr->rateCategory[i]];

					assert(
							0 <= tr->rateCategory[i] && tr->rateCategory[i] < tr->maxCategories);
				...
				for (i = 0; i < tr->partitionData[model].numberOfCategories;
						i++)
...
#if NOT (defined(_FINE_GRAIN_MPI) || defined(_USE_PTHREADS))
			{
				...
				for (i = lower, localCount = 0; i < upper; i++, localCount++) {
					tr->partitionData[model].rateCategory[localCount] =
							tr->rateCategory[i];
				}
			}
#endif
		...
		if (scaleRates) {
			for (model = 0, accRat = 0.0, accWgt = 0;
					model < tr->NumberOfModels; model++) {
				int localCount = 0, lower = tr->partitionData[model].lower,
		...
		for (model = 0; model < tr->NumberOfModels; model++) {
			int localCount = 0, lower = tr->partitionData[model].lower, upper =
					tr->partitionData[model].upper;

		}
#if NOT (defined(_FINE_GRAIN_MPI) || defined(_USE_PTHREADS))
		for (model = 0; model < tr->NumberOfModels; model++) {
			int localCount, lower = tr->partitionData[model].lower, upper =
					tr->partitionData[model].upper;
...
static void optimizeRateCategories(tree *tr, int _maxCategories) {
	...
	if (_maxCategories > 1) {
		double temp, lower_spacing, upper_spacing, initialLH = tr->likelihood,
				*ratStored = (double *) malloc(
						sizeof(double) * tr->originalCrunchedLength),
				/**lhs =       (double *)malloc(sizeof(double) * tr->originalCrunchedLength),*/
				**oldCategorizedRates = (double **) malloc(
						sizeof(double *) * tr->NumberOfModels);

		int i, k, maxCategories = _maxCategories, *oldCategory = (int *) malloc(
				sizeof(int) * tr->originalCrunchedLength), model, *oldNumbers =
				(int *) malloc(sizeof(int) * tr->NumberOfModels);
		...
		if (tr->optimizeRateCategoryInvocations == 1) {
			lower_spacing = 0.5
					/ ((double) (tr->optimizeRateCategoryInvocations));
			upper_spacing = 1.0
					/ ((double) (tr->optimizeRateCategoryInvocations));
		} else {
			lower_spacing = 0.05
		...
		memcpy(oldCategory, tr->rateCategory,
				sizeof(int) * tr->originalCrunchedLength);
		memcpy(ratStored, tr->patratStored,
				sizeof(double) * tr->originalCrunchedLength);

		for (model = 0; model < tr->NumberOfModels; model++) {
			oldNumbers[model] = tr->partitionData[model].numberOfCategories;

			oldCategorizedRates[model] = (double *) malloc(
					sizeof(double) * tr->maxCategories);

			memcpy(oldCategorizedRates[model],
					tr->partitionData[model].perSiteRates,
					tr->maxCategories * sizeof(double));
		}
		...
		for (model = 0; model < tr->NumberOfModels; model++) {
			int where = 1, found = 0, width = tr->partitionData[model].upper
					- tr->partitionData[model].lower, upper =
					tr->partitionData[model].upper, lower =
					tr->partitionData[model].lower;

			rateCategorize *rc = (rateCategorize *) malloc(
					sizeof(rateCategorize) * width);
		...
		if (tr->likelihood < initialLH) {
			for (model = 0; model < tr->NumberOfModels; model++) {
				...
				memcpy(tr->partitionData[model].perSiteRates,
						oldCategorizedRates[model],
						tr->maxCategories * sizeof(double));
			}

			memcpy(tr->patratStored, ratStored,
					sizeof(double) * tr->originalCrunchedLength);
			memcpy(tr->rateCategory, oldCategory,
					sizeof(int) * tr->originalCrunchedLength);
